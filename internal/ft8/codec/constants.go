// Package codec
// constants.go — LDPC(174,91) matrix data for FT8/FT4.
//
// All arrays are transcribed from ft8_lib (MIT license) constants.c and
// WSJT-X LDPC_174_91_3_generator.f90. Values in Mn and Nm are 1-indexed
// to keep them auditable against the reference sources; encoder/decoder
// code adjusts to 0-indexed in the loop.
//
// Naming follows standard LDPC / ft8_lib convention:
//
//   - Mn[n] = check-node neighbours of variable node n  (ft8_lib kFTX_LDPC_Mn)
//   - Nm[m] = variable-node neighbours of check node m  (ft8_lib kFTX_LDPC_Nm)
//
// Reference: ft8_lib commit HEAD, WSJT-X lib/ft8/.

package codec

const (
	// N is the codeword length (total coded bits).
	N = 174
	// K is the number of information bits (77 message + 14 CRC).
	K = 91
	// M is the number of parity bits (N − K).
	M = 83

	// NBytes is the number of bytes needed to hold N bits (⌈174/8⌉ = 22).
	NBytes = (N + 7) / 8
	// KBytes is the number of bytes needed to hold K bits (⌈91/8⌉ = 12).
	KBytes = (K + 7) / 8
)

// Mn contains the Tanner-graph adjacency from variable nodes to check nodes.
// For each of the 174 variable (codeword-bit) nodes, three check-node
// neighbours are listed. All values are 1-indexed.
//
// Source: ft8_lib kFTX_LDPC_Mn[174][3].
var Mn = [N][3]uint8{
	{16, 45, 73},
	{25, 51, 62},
	{33, 58, 78},
...
// Nm contains the Tanner-graph adjacency from check nodes to variable nodes.
// For each of the 83 check (parity) nodes, up to 7 variable-node neighbours
// are listed. Unused trailing positions are zero. All active values are
// 1-indexed.
//
// Source: ft8_lib kFTX_LDPC_Nm[83][7].
var Nm = [M][7]uint8{
...
// NmCount holds the degree (number of active variable-node neighbours) for
// each of the 83 check nodes. Values are either 6 or 7.
//
// Source: ft8_lib kFTX_LDPC_Num_rows[83].
var NmCount = [M]uint8{
...
// G is the generator matrix for the (174,91) LDPC code. Each of the 83 rows
// contains 91 bits packed MSB-first into 12 bytes (96 bits total; the 5
// unused low-order bits of the 12th byte are always zero).
//
// To compute parity bit p (0 ≤ p < 83), take the GF(2) dot product of G[p]
// with the 91-bit information vector: popcount(G[p] AND info) mod 2.
//
// Source: WSJT-X LDPC_174_91_3_generator.f90, ft8_lib kFTX_LDPC_generator.
var G = [M][KBytes]byte{
...
// TODO: FT4 matrices — FT4 uses the same (174,91) code structure but may
// require different generator/parity matrices. For now only FT8 tables are
// provided.
