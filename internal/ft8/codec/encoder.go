...
// by [message.Append91]). The 5 unused low-order bits of info[11] must be zero.
//
...
// The 6 unused low-order bits of the 22nd byte are always zero.
func Encode(info [KBytes]byte) [NBytes]byte {
	...
	// Step 2: Compute 83 parity bits via the generator matrix G.
	// For each parity bit p (0..82), compute the GF(2) dot product of
	// generator row G[p] with the 91-bit information vector:
	...
	for p := range M {
		...
		// Step 3: Write parity bit p into codeword position K+p (91+p).
