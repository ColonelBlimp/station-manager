...
// sampleRing is a fixed-capacity wrapping buffer of int16 samples used
// by the slot scheduler to hold "the last N samples we've seen" without
// per-slot allocation. int16 because that's the seam DecodeSlot (and
// go-ft8) consumes — the whole live pipeline stays int16 end to end.
//
// Concurrency: not safe for concurrent use. The scheduler owns the ring
// and serialises Append / Snapshot on a single goroutine.
type sampleRing struct {
	buf    []int16
	w      int   // next write index in buf
	filled int64 // total samples ever written
}

// newSampleRing allocates a ring of the given capacity.
func newSampleRing(size int) *sampleRing {
	return &sampleRing{buf: make([]int16, size)}
}
...
func (r *sampleRing) Append(src []int16) {
	n := len(src)
	if n == 0 {
		return
	}
...
// recent Cap() samples in chronological order (oldest first). If fewer
// than Cap() samples have been appended, the head of the returned slice
// is zero-padded.
func (r *sampleRing) Snapshot() []int16 {
	size := len(r.buf)
	out := make([]int16, size)
