...
type Qso struct {
	...
	// Revision is the row's monotonic edit counter (ADR 0050) — the sync
	// protocol's version marker, bumped by the same stamp trigger that owns
	// ModifiedAt. Same column-only class as ModifiedAt/DeletedAt (`json:"-"`,
	// envelope-carried on the smcloud wire): second-precision wall-clock
	// cannot order same-second edits, revision can. 0 = pre-revision row
	// (the cloud guard falls back to modified_at on ties).
	Revision int64 `json:"-"`
...
type QsoManifestEntry struct {
	...
	// Revision is the row's monotonic edit counter (ADR 0050); reconcile
	// treats a revision mismatch as drift even when timestamps tie.
	Revision int64
	Deleted  bool
}
