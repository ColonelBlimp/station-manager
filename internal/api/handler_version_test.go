...
func TestVersion_HappyPath(t *testing.T) {
	...
	// currently 2 (0001_init + 0002_relax_rst_length).
	if !strings.Contains(body, `"schema":{"version":2,"dirty":false}`) {
		t.Fatalf("body = %q, want schema:{version:2,dirty:false}", body)
	}
}
