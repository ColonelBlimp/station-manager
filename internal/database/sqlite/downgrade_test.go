...
func TestDowngradeLogSchemaTo_DownToPriorVersion_RetainsRows(t *testing.T) {
	...
	if v := schemaVersion(t, svc); v != 12 {
		t.Fatalf("schema version = %d, want 12 (head)", v)
	}
	...
	if from != 12 {
		t.Fatalf("reported from = %d, want 12", from)
	}
