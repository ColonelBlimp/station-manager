...
func TestDowngradeLogSchemaTo_DownToPriorVersion_RetainsRows(t *testing.T) {
	...
	if v := schemaVersion(t, svc); v != 15 {
		t.Fatalf("schema version = %d, want 15 (head)", v)
	}
	...
	if from != 15 {
		t.Fatalf("reported from = %d, want 15", from)
	}
