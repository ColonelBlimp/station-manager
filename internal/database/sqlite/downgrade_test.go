...
func TestDowngradeLogSchemaTo_DownToPriorVersion_RetainsRows(t *testing.T) {
	...
	if v := schemaVersion(t, svc); v != 11 {
		t.Fatalf("schema version = %d, want 11 (head)", v)
	}
	...
	if from != 11 {
		t.Fatalf("reported from = %d, want 11", from)
	}
...
func TestDowngradeLogSchemaTo_RefusesSameOrHigherTarget(t *testing.T) {
	svc := testService(t)
	for _, target := range []uint{11, 12, 99} {
	...
	if _, err := svc.DowngradeLogSchemaTo(10); err != nil {
		t.Fatalf("downgrade to 10: %v", err)
	}
	if _, err := svc.DowngradeLogSchemaTo(11); err == nil {
		t.Fatal("target 11 accepted from version 10; the command migrated UP")
	}
	if v := schemaVersion(t, svc); v != 10 {
		t.Fatalf("schema version = %d after the refused upward target, want 10", v)
	}
}
