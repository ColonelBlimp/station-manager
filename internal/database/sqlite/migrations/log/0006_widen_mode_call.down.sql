-- Revert 0006: restore the narrower `mode` (<= 10) and `call` (1..20) CHECKs.
--
-- Rebuilt from the POST-0005 schema, same as the up: the `revision` column and the
-- revision-aware trg_qso_set_modified_at trigger are carried forward. This rollback
-- FAILS (by design) if any stored row now holds a mode > 10 chars or a call > 20 —
-- a schema can't narrow below the data it already contains; drop or fix those rows
-- before rolling back.
