-- Widen the `mode` and `call` CHECK constraints to admit the full set of values
-- the validation layer already accepts (2026-07-21 qsoservice review, finding 1).
--
-- Two catalog-valid records could pass qsoservice validation but then violate a
-- narrower column CHECK on insert — surfacing as a 500 on a live submit and, worse,
-- ABORTING a bulk import (importBatchFallback treats a CHECK violation as a hard DB
-- error, not a per-record SubmitError), so ONE such QSO in a QRZ ADIF sinks the
-- whole restore:
--   * mode:  DIGITALVOICE is a real ADIF MODE (12 chars) and DSTAR/DMR/C4FM submodes
--            resolve to it, but the old CHECK capped mode at 10 → widen to 20 (room
--            for DIGITALVOICE plus headroom for operator-added modes.json entries).
--   * call:  IsValidCallsign accepts 3..32 chars (and logbook.callsign is already
--            3..32), but the old contacted-station `call` CHECK capped it at 20 →
--            widen to 32 to match.
--
-- Only these two CHECKs change. Same rename-then-rebuild-all-three shape as
-- 0002/0004 (SQLite cannot ALTER a CHECK, and rebuilding qso alone would leave the
-- qso_upload / qso_history foreign keys pointing at the dropped qso_old). Rebuilt
-- from the POST-0005 schema: the `revision` column (0005) and the revision-aware
-- trg_qso_set_modified_at trigger (0005) are carried forward unchanged; the UTC
-- datetime('now') defaults (0004) are preserved.
...
(
    ...
    call            TEXT     NOT NULL CHECK (length(trim(call)) BETWEEN 1 AND 32),
    band            TEXT     NOT NULL CHECK (length(trim(band)) <= 10),
    mode            TEXT     NOT NULL CHECK (length(trim(mode)) <= 20),
...
INSERT INTO qso (
    ...
    additional_data, dedupe_key, logbook_id, revision
)
SELECT
    id, created_at, modified_at, deleted_at, uuid, call, band, mode, freq,
    qso_date, time_on, time_off, rst_sent, rst_rcvd, country,
...
-- Recreated from 0005 (revision-aware): owns BOTH the modified_at and revision
-- stamps in one inner UPDATE, self-terminating under recursive_triggers.
