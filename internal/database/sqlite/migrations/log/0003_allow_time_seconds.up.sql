-- Relax the time_on / time_off CHECK to accept HHMMSS (6-digit) in addition to
-- HHMM (4-digit), per the ADIF spec (either is valid). SM stops truncating
-- seconds so FT8's real slot seconds and imported HHMMSS survive — the QSL
-- manager (M0URX/OQRS) matches on the full timestamp. Dedupe stays minute-
-- precision in Go, so this is purely a storage-precision relaxation.
--
-- Same rename-then-rebuild shape as 0002 (SQLite can't ALTER a CHECK): rebuild
-- qso with the relaxed constraint, then rebuild the FK-referencing tables so
-- their foreign keys re-point at the new qso — no foreign_keys pragma toggling
-- (which is a no-op inside migrate's transaction).
