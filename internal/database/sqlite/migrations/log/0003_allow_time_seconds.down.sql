-- Revert: re-impose the HHMM-only (4-digit) time_on / time_off CHECK. Any stored
-- HHMMSS values are truncated to HHMM on the way back (substr(...,1,4)) so the
-- down migration can't fail on 6-digit data. Same rebuild shape as the up.
...
SELECT
    id, created_at, modified_at, deleted_at, uuid, call, band, mode, freq,
    qso_date, substr(time_on, 1, 4), substr(time_off, 1, 4), rst_sent, rst_rcvd, country,
    additional_data, dedupe_key, logbook_id
FROM qso_old;
