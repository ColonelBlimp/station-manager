-- Reverse 0004: restore the `datetime('now','localtime')` DEFAULTs + trigger bodies
-- and shift the normalised rows back to local (CAT, UTC+2). Best-effort, and lossy:
-- the up migration already dropped sub-second precision on the rows it shifted, so
-- a down→up round-trip won't restore it. Canonical-UTC rows (`…+00:00`, from
-- sqlboiler / the Go writers) were UTC before 0004 too and are left untouched.
