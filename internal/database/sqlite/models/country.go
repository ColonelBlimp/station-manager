...
func Countries(mods ...qm.QueryMod) countryQuery {
	mods = append(mods, qm.From("\"country\""), qmhelper.WhereIsNull("\"country\".\"deleted_at\""))
	q := NewQuery(mods...)
...
func FindCountry(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*Country, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"country\" where \"id\"=? and \"deleted_at\" is null", sel,
	)
...
func (o *Country) Delete(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	...
	if hardDelete {
		...
		sql = fmt.Sprintf("UPDATE \"country\" SET %s WHERE \"id\"=?",
			strmangle.SetParamNames("\"", "\"", 0, wl),
		)
		valueMapping, err := queries.BindMapping(countryType, countryMapping, append(wl, countryPrimaryKeyColumns...))
		if err != nil {
			return 0, err
		}
...
func (q countryQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if q.Query == nil {
...
func (o CountrySlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	...
	if hardDelete {
		...
		wl := []string{"deleted_at"}
		sql = fmt.Sprintf("UPDATE \"country\" SET %s WHERE "+
			strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, countryPrimaryKeyColumns, len(o)),
			strmangle.SetParamNames("\"", "\"", 0, wl),
		)
...
func CountryExists(ctx context.Context, exec boil.ContextExecutor, iD int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"country\" where \"id\"=? and \"deleted_at\" is null limit 1)"
