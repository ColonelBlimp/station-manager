...
// count and a SHA-256 hex hash of the sorted (uuid, modified_at) pairs.
// Canonicalisation, so both ends agree byte-for-byte:
//
...
//
// The hash input is one "uuid|unixmicro\n" line per entry; zero entries hash
// to SHA-256 of the empty string. Input order is irrelevant (Summary sorts a
// copy; the caller's slice is untouched).
