...
func TestDBDowngrade_MigratesLogSchemaDownAndReports(t *testing.T) {
	...
	if !strings.Contains(out.String(), "13 → 10") {
...
func TestDBDowngrade_RefusesWithoutYesAndRefusesUpward(t *testing.T) {
	...
	if err := runDBDowngradeTo(&out, []string{"--config", cfgPath, "--to", "13", "--yes"}); err == nil {
	...
	if err != nil || v != 13 {
		t.Fatalf("schema version = %d (%v) after refusals, want 13 untouched", v, err)
	}
}
