...
- **FT8 subsystem** (`internal/ft8`) — **operator-initiated**: every session starts with an operator action (the click + arming TX) and the daemon never initiates one. Daemon-initiated sequencing is out of scope — the QEX FT8 spec forbids automatic operation and unattended operation is licence-restricted in many jurisdictions. Note the precise claim, which has two halves and is easy to get half-right: SM does not *require* the operator to remain once a run is started (a Call-CQ run works answerers until Abandon, an ADR 0059 auto-work run works callers), so it is not "attended-only" in any enforced sense — staying at the desk is the operator's responsibility under their licence. **But one presence check IS enforced: the FT8 view must stay open.** When the last `/v1/ft8/events` subscriber is gone past the linger window, `Service.onLingerExpired` disarms TX — dropping PTT and abandoning any active QSO — before releasing the capture device (`internal/ft8/service.go:398–429`; the code calls this the "attended-only guarantee", which is where the over-broad word came from — it enforces an open SSE subscription, not a human). So: walking away with the browser open does not stop a run; closing the browser does. Never write "nothing stops it", and never write "attended-only". Links **go-ft8** (`github.com/ColonelBlimp/go-ft8`, GPL-3.0-only — the trigger for SM's GPL-3.0-only licence; see the Licence note below). A bare decode is never a QSO; a completed *exchange* is. TX (ADR 0029–0033) reuses the bridge's guaranteed-stop discipline (`tx_on`/`tx_off` never `exposed`); manual sequencing only. **Live FT8 requires a CGO build** (the static CGO-free default leaves the subsystem idle). `internal/ft8` stays narrow by import graph (no `qsoservice`/`config`/`adif` import; assembly + submit live in an injected sink). Full design/decision detail in **`internal/ft8/CLAUDE.md`** (auto-loads when working there); the single FT8 capture point is **`docs/ft8.md`**.
