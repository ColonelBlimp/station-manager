...
- **Use `internal/errors` for operation-tagged errors.** The `errors.New(op).Err(err).Msg(...)` pattern is this project's canonical idiom. Prefer it over `fmt.Errorf` in code that belongs to the tagged-error paths.
