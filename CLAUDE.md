...
- **`internal/bridge` subsystem** per ADR 0013 + ADR 0019 — daemon subsystem providing `/v1/rig/events` SSE for the logging SPA. v1 is **read-only**: rig pushes state via AUTO-mode CAT, bridge filters and forwards via SSE, SPA displays. **M3a (bridge subsystem v1) closed 2026-05-11** — all four sub-milestones shipped (M3a.1 skeleton + SSE plumbing, M3a.2 real serial+CAT pipeline, M3a.3 bootstrap-on-SSE-open via rigdef-driven `READ` + `bridge-error` events with hub cache for late subscribers + identity verification, M3a.4 SPA `bridge.svelte.ts` EventSource consumer + live rig test on the FTdx10). Daemon-authoritative CAT-enable: SPA hydrates `configState.station.enabled` from `/v1/config` `bridge.enabled`; `/v1/config` also exposes `bridge.rig_name` (resolved daemon-side from `cat.Lookup(driver).Name`, e.g. "Yaesu FTdx10") for Equipment-panel display + ADIF MY_RIG fallback. Default deployment is single-binary (daemon imports the bridge package); split-host `cmd/bridge` is opt-in and parked. **Parked work:** rig-specific mode → ADIF translation (DATA-U / CW-U / RTTY-U etc. don't map 1:1 to the daemon's ADIF enum — operator decision: per-rig translation table in rigdef JSON; design deferred to a follow-up session, full context in `docs/v2-design/cat-serial-reuse.md §8`). **Not in v1:** PTT, inbound command path, rigctld-compat TCP, NDJSON Unix-socket frontend, persistent rig-state cache (the only hub cache is one slot for `bridge-error` replay to late subscribers).
