...
- **FT8 removed from the SM tree 2026-05-30 — now a separate stream.** The `internal/ft8` subsystem (codec + DSP + decode/ring/scheduler/service), the `research/` clean-room decoder tree (sandbox at strict 129/18 matched), the `captures/` corpus, the `cmd/ft8-*` developer tools, and `internal/audio/capture` (CGO miniaudio) were removed and preserved at tag **`ft8-snapshot-2026-05-30`** — the whole tree is recoverable (`git checkout ft8-snapshot-2026-05-30 -- <paths>`). FT8 work continues out-of-tree: a separate repo is planned, with a fresh clean-room MIT v2 building on the research learnings, later either imported as a library or re-inlined. **`internal/audio`** (CGO-free WAV/FFT) was deliberately RETAINED as a self-contained package for future operator-facing recording/playback. Durable constraints for the future lib (still authoritative): clean-room from the QEX 2020 paper + ref [14] public-domain tarball ONLY, never the GPL WSJT-X Fortran source; `jt9`/`ft8sim` are black-box test oracles only; CGO GPL (FFTW3) triggers at BINARY distribution, so prefer BSD/MIT FFT (KissFFT/PocketFFT) on any distributable path. Full rules in ADR 0021 (parked) + `docs/licensing.md`.
