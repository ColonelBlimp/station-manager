...
// message: c28 r1 c28 r1 R1 g15 i3.
//
//	bits[0..27]  c28 — first callsign (CQ/DE/QRZ, hash, or standard)
//	bits[28]     r1  — first call /R rover-suffix flag
//	bits[29..56] c28 — second callsign
//	bits[57]     r1  — second call /R rover-suffix flag
...
//
// Output format mirrors jt9: "CALL1 CALL2 EXTRA", with /R appended
// to either call when its r1 bit is set, and R prepended to EXTRA
// when R1 is set.
//
// The r1 suffix is /R (rover) on Type 1 per QEX paper §2 Table 1.
// /P (portable) appears on Type 2 (EU VHF contest, i3=2), which
// this unpacker doesn't yet handle. Earlier versions of this file
// incorrectly rendered Type 1's r1 as /P — fixed 2026-05-26 per
// code review. No effect on the current 144-signal real-capture
// corpus (0 /R signals in the jt9 oracle), but rover messages
// would previously have rendered with wrong suffix.
func unpackType1(p []uint8) (string, error) {
	...
	if extra != "" {
		...
		// R-prefix flag set but g15 blank — empty "R" report. Rare; emit "R" alone.
		sb.WriteString(" R")
	}
