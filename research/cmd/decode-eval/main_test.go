...
// BP-only baseline captured 2026-05-25 before OSD fallback.
// These are intentionally a floor, not exact golden counts — the
// purpose is to catch regressions in the BP+CRC pipeline while
// leaving room for OSD-2 and later upgrades to raise the matched
// counts without test churn.
//
...
//     totals): "at least this many" assertions. OSD-2 work should
//     raise these; a drop fails the suite.
//
...
const (
	...
	// Strict tier: clean and -16 dB decode at 100% under BP-only.
	syntheticCleanMatchedExact = 10
	synthetic16dBMatchedExact  = 10
	syntheticStrictExtraExact  = 0

	// Floor tier: -20 / -22 dB are where BP saturates without OSD.
	// -20dB: BP-only gets 3/10. -22dB: BP-only gets 0/10. OSD-2
	// should raise both; no extras ceiling here so OSD work has
	// room to iterate on the marginal-SNR regime.
	synthetic20dBMatchedFloor = 3
	synthetic22dBMatchedFloor = 0

	// Real-capture corpus aggregate vs jt9-oracle truth:
	//   88 / 144 matched (61.1% decode parity)
	//    1 extra (live_slot3, inconclusive — slot-bleed or rare
	//      CRC false-accept, unresolvable without Layer-2 unpacking)
	realCaptureTruthTotal   = 144
	realCaptureMatchedFloor = 88
	realCaptureExtraCeiling = 1
)
...
// where the single observed extra is the known outlier.
var realCaptureSlots = []struct {
	...
	{"../../../captures/20m_slot1.wav", 21, 15, 0},
	{"../../../captures/20m_slot2.wav", 32, 18, 0},
	{"../../../captures/20m_slot3.wav", 17, 9, 0},
	{"../../../captures/live_slot1.wav", 29, 16, 0},
	{"../../../captures/live_slot2.wav", 23, 17, 0},
	{"../../../captures/live_slot3.wav", 22, 13, 1},
}
...
func TestSyntheticBaseline(t *testing.T) {
	...
	t.Log("synthetic baseline (BP-only + incoherent demod):")
...
func TestRealCaptureBaseline(t *testing.T) {
	...
	// Always log the table — visible with -v and on any failure.
	t.Log("real-capture baseline (BP-only + incoherent demod):")
