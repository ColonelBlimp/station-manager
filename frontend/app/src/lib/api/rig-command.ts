/*
    ...
    command path). Sends one semantic op + scalar value and reports the outcome
    as a discriminated union, same conventions as lib/api/rig-tune.ts.
 ...
 * for valueless ops (swap_vfo / band_up / band_down). Passed through verbatim
 * (the daemon renders it to the wire string). Returns 'ok' on the daemon's 202
 * Accepted; the new rig state arrives over the SSE stream, not in this response.
 */
