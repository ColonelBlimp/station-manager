...
<script lang="ts">
    ...
    let { origin = null, arcs = [] }: Props = $props();
    ...
    const drawnArcs = $derived(
        arcs.flatMap((a) => {
            ...
            return d === null || end === null ? [] : [{ key: a.key, label: a.label, d, end }];
        })
    );
</script>
...
>
    {#if spherePath !== null}
        <path d={spherePath} class="fill-canvas stroke-line" stroke-width="1" />
    {/if}
    {#if gratPath !== null}
    ...
    {#each countries as c (c.id)}
        {#if c.d !== null}
            <path
                d={c.d}
                data-testid="country"
                class="fill-surface-muted stroke-line"
                stroke-width="0.5"
            />
        {/if}
    {/each}
    {#each drawnArcs as a (a.key)}
        <g data-testid="arc">
            ...
            <circle cx={a.end[0]} cy={a.end[1]} r="3" class="fill-focus">
                {#if a.label}<title>{a.label}</title>{/if}
            </circle>
        </g>
    {/each}
