...
- **State:** station on `2.0.0-alpha.2-90-g7126ab64`. Contest record entry 37; SARL log submitted. 2026-09-13 on air: 150 QSOs, 0 still-keyed alarms. Deployed 2026-09-14: one `/v1/events` per tab, no rig stream in a Map tab, Dashboard retired (root → last-used Operate mode). Undeployed: `e409b842`. **W-0020 Station Events SELECTED**, review package approved 2026-09-14.
- **Next:** operator reviews W-0020, then slice 1 (store). Passive: Map tab beside FT8 → 4 subscribers. W-0011: one more long session. Rulings open: repeat-hold memory; landing-view preference. Then G3/AC7 (per-occasion) or waive; start-failure visibility; Excel export; next candidate freeze. RF only by per-occasion agreement; FT8-10 BLOCKED.
