...
- **State:** W-0010 outcome 9 CLOSED 2026-09-21 (deployed `alpha.3-17`, schema 11). W-0021 rulings (a)–(e) settled; slice 1 sub-commits A (`smd db-downgrade`) and B (config v4 catalogue and `smd config-downgrade`) built. Station on a dev build; frozen RPM deferred.
- **Next:** the paired rollback drill on isolated copies, then slice 1 sub-commit C (migration 0012 `archive_metadata` + `logbook.uuid`, adoption, `default_logbook_id` projection) RED-first. Order: archives → Settings → Logbooks → contesting; W-0012's 3 slices independent. **alpha.3 FROZEN** at `333427ea`; [record](reports/dogfood-acceptance-v2.0.0-alpha.3.md) Gate A pending. W-0020 AC2 awaits passive rows. RF per-occasion only; FT8-10 BLOCKED.
