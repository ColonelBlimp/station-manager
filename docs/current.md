...
- **Next:** Operator rules the B1/A5-03 rows and triages Findings 1–7 (record + dogfood inbox); B2 surface inventory in Firefox per the record; Finding 6 must be fixed in the next candidate or explicitly waived before dogfood acceptance. Backlog candidates: load-time `action_filter` reconciliation, `config-check` running full Validate, false TX alarm at bridge open, SM Cloud https on the LAN. Hardware/rig-command/RF stay unauthorized; FT8-10 BLOCKED. Hardware/rig-command/RF remain unauthorized; FT8-10 stays BLOCKED.
