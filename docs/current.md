...
- **State:** outcome 9 slices 1 (QRZ no-op delete, ADR 0081) and 2 (`failure_class`, migration 0011, uncommitted) built 2026-09-20; rulings (a)–(d) in the dossier. Station on dev build `2.0.0-alpha.3-1-gbe613793`; frozen RPM install deferred.
- **Next:** slice 3, boot re-arm of `auth` failed rows; then slice 4, card and API (waiting/failed counts, retry). Order (2026-09-19): alpha.3 acceptance → outcome 9 → ADR 0071 archives (files first) → Settings → Logbooks → contesting; W-0012's 3 slices independent. **alpha.3 FROZEN** at `333427ea` (local tag); [record](reports/dogfood-acceptance-v2.0.0-alpha.3.md) Gate A pending the operator. W-0020 AC2 awaits passive rows. RF per-occasion only; FT8-10 BLOCKED.
