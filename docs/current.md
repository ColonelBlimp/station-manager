...
- **State:** station on `2.0.0-alpha.2-65-gba2abec7`. Contest worked 2026-09-12 15:00–17:14Z: 51 QSOs (40 on 20 m, 11 on 40 m), 458 rungs, every upload first attempt — record entry 37. G2/G4 met; AC7/G3 waived until the post-contest measurements. Built, NOT deployed: re-sent stop before any TX alarm + ms log stamps (W-0011); FT-view Mode readout (W-0012).
- **Next:** one deploy (`task deploy:local:dev`) before the operator's FT8/FT4 session, then watch the next still-keyed event and the Mode readout. Operator: SARL log due Thu 2026-09-17 21:59Z. Then the same-band dupe ruling (W-0019); G3 dummy-load + AC7 window (per-occasion); inbox triage (map on the shell stream + enrichment cap, filter indicator, start-failure visibility, Excel export, draft persistence, QRZ `last_error` key echo). Then the next candidate freeze. RF only by per-occasion agreement; FT8-10 BLOCKED.
