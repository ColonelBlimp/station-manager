...
- **State:** W-0021 slice 1 SHIPPED and deployed `alpha.3-24` (schema 12, station adopted as legacy archive, config v4; `pre-w0021` tag = rollback build). Slices 2A–2D, 3 and 4 pushed, CI green (slice 4 = SPA Archives tab, header selector, fail-closed switch gate; last `8b0ee330`). Not yet deployed.
- **Next:** deploy + AC 1 station drills (operator, later session: real second archive, activate, log, switch back); then slice 5 (SM Cloud identity). Then Settings → Logbooks → contesting; W-0012 independent. **alpha.3 FROZEN** at `333427ea`; [record](reports/dogfood-acceptance-v2.0.0-alpha.3.md) Gate A pending. W-0020 AC2 awaits passive rows. RF per-occasion only; FT8-10 BLOCKED.
