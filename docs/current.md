...
- **State:** W-0010 outcome 9 CLOSED 2026-09-21. W-0021 slice 1 complete through C (migration 0012, identity, in-place adoption); the paired rollback drill passed at schema 12 for the pre-W-0021 tag and alpha.3. Station still on a dev build at schema 11; frozen RPM deferred.
- **Next:** deploy the slice 1 dev build and read `GET /v1/version` `archive` on the station; then slice 2 (global stores, provisioner, last-known-good start, forwarding gate, import/restore resolver). Then Settings → Logbooks → contesting; W-0012 independent. **alpha.3 FROZEN** at `333427ea`; [record](reports/dogfood-acceptance-v2.0.0-alpha.3.md) Gate A pending. W-0020 AC2 awaits passive rows. RF per-occasion only; FT8-10 BLOCKED.
