...
- **Next:** operator's on-screen tune check; operator reviews W-0020, then slice 1 (store). Separate de-flake of the confirm-timeout test. Open rulings: repeat-hold memory; landing view. Then G3/AC7 (per-occasion) or waive; start-failure visibility; Excel export; next freeze. RF per-occasion only; FT8-10 BLOCKED.
