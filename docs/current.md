...
- **State:** W-0021 slice 1 SHIPPED and deployed `alpha.3-24` (schema 12, station adopted as legacy archive, config v4; `pre-w0021` tag = rollback build). Slice 2A committed (`cc1078b7`, `50ae1e6c`); 2B (interim forwarding gate) built, uncommitted.
- **Next:** commit 2B; then 2C managed provisioner, 2D pending→active start with last-known-good rollback. Then Settings → Logbooks → contesting; W-0012 independent. **alpha.3 FROZEN** at `333427ea`; [record](reports/dogfood-acceptance-v2.0.0-alpha.3.md) Gate A pending. W-0020 AC2 awaits passive rows. RF per-occasion only; FT8-10 BLOCKED.
