...
- **Next:** Reconcile the code-current P1 findings the action-open internal audits report into the ranked backlog — the persistence-transaction audit flags PT-1/PT-2 as open, contradicting `docs/backlog.md`'s "no P1 remains" — verifying each against code, for the operator to rank before any P1 dossier IDs are assigned. Session-email templating waits behind that.
