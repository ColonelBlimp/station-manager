...
- **Next:** Ready to deploy is PASS. Remove the retired `fedora44` image/override. Verify the frozen RPM SHA (do not rebuild from docs-only HEAD), install it live from the stopped daemon, daemon-reload/start, then check version, migration, QSO/queue counts and basic UI; the operator's intended second pass with the old daemon running must be defined first. Hardware/rig-command/RF remain unauthorized; FT8-10 stays BLOCKED.
