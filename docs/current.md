...
- **State:** Documentation routing is live. All three legacy operator SPAs are retired under preservation tags, and `frontend/app` is the sole embedded operator SPA. [W-0003](work/W-0003-retire-legacy-operator-spas.md) remains open for the canonical-root and route-level lazy-loading acceptance work.
- **Next:** Re-verify and reduce the canonical config reference to a current contract; then migrate the next eligible ranked open item into `W-0005`.
