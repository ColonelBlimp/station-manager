...
- **State:** W-0021 slice 1 SHIPPED and deployed `alpha.3-24` (schema 12, station adopted as legacy archive, config v4; `pre-w0021` tag = rollback build). Slices 2A–2C pushed (2C `8c8af208`…`c8ac3c4e`); 2D (pending→active start as fresh-container generations with last-known-good fallback, `archive-promote` node) built, uncommitted.
- **Next:** commit 2D; then slice 3 (create/list/activate API over the attended restart). Then Settings → Logbooks → contesting; W-0012 independent. **alpha.3 FROZEN** at `333427ea`; [record](reports/dogfood-acceptance-v2.0.0-alpha.3.md) Gate A pending. W-0020 AC2 awaits passive rows. RF per-occasion only; FT8-10 BLOCKED.
