...
- ✅ `internal/bridge` package per ADR 0013 + ADR 0019 — daemon subsystem providing rig SSE on `/v1/rig/events`. **v1 is read-only** (rig pushes state via AUTO-mode CAT, bridge filters and forwards to the logging SPA; no PTT, no inbound command path, no rigctld-compat TCP, no NDJSON Unix-socket frontend). All four sub-milestones (M3a.1–M3a.4) shipped 2026-05-10/11 — bridge subsystem v1 is closed. Rig-mode → ADIF translation shipped session 51 (2026-05-11) via a per-rig two-layer table (rigdef-shipped defaults + operator overrides merged at `/v1/config`) and a My Station Mode Mappings sub-tab; bridge stays a pure pass-through, SPA resolves. Default deployment is single-binary; split-host `cmd/bridge` is opt-in and parked. Eventual feature set (rigctld TCP, PTT, etc.) builds when its drivers appear.
