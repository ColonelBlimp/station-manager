...
**Why this matters:** v1 conflated data capture with storage/forwarding. Adding WSJT-X UDP ingest and FT8 support exposed this as a structural mistake — FT8 was eventually extracted to its own repo to escape the coupling, WSJT-X integration hit a serial-contention wall, and the logging app accumulated responsibilities that don't belong together. v2's decomposition (daemon + clients + bridge) exists specifically to avoid re-making this mistake. **Updated 2026-05-16:** ADR 0021 brought FT8 back in-tree as `internal/ft8/`, a daemon subsystem with package-boundary discipline (same shape as `internal/bridge`); the narrow-scope invariant is satisfied by the **import graph**, not by process or repo separation. **Updated 2026-05-30:** ADR 0021 was parked — the in-tree clean-room FT8 was moved out (tag `ft8-snapshot-2026-05-30`). **Updated 2026-05-31 (ADR 0024):** FT8 is back in-tree, but as a *thin wrapper around an external decoder*, not the in-process clean-room subsystem 0021 envisaged — SM links the external **go-ft8** library (GPL) and `internal/ft8` is the wrapper + live capture/scheduler/Service pipeline. The *invariant is still satisfied by the import graph*: `internal/ft8` does not touch `internal/storage`/`internal/forwarder`, and **a decode is not a QSO** — the subsystem only logs "heard this" lines, writing nothing to the QSO table or upload queue. The bridge half of the import-graph rule (`internal/storage`/`internal/forwarder` ↔ `internal/bridge` mutually non-importing) remains live. The legitimate future integration point — operator turning a decode into a logged QSO — would be `internal/qsoservice` (`qsoservice.Submit`), not a coupling violation.
