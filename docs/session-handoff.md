...
### Session 108 (2026-05-30) — **Unresolved-hash emission promoted as sandbox strict-mode default: 113/17 → 128/19 (+15 matched / +2 extras), the largest single decoder-side lift of the cluster.** Strict baseline is now **128/144 = 88.9% matched**. The library default (`DefaultMultiPassOptions`) is UNCHANGED — strict-profile-only promotion; `TestCorpusBaseline` still pins 111/23.
...
**Next session pickup — bracket-convention fix (SEPARATE unit, own A/B).** jt9 renders hash-*resolved* calls as `<CALL>` (brackets retained); we render bare `CALL`. The `DG6JW/T SV0TPN +01` extra is entirely this — emitting `<DG6JW/T>` would flip it extra→match. Changes display semantics for resolved hash-origin calls, may affect more than the one obvious case → needs independent match/extras A/B + a trace of all `<CALL>`-eligible outputs. Do NOT bundle with anything else. Residual 7 decoder-bound after emit-unresolved: 3 genuine real-signal BP failures (`CQ DX S56GD`×2, `CQ COTA IK7NXU`), 2 noise-limited, 1 Type-4 CQ form, 1 jt9-resolved-bracket mismatch. Report: `research/sandbox/reports/unresolved-hash-emit-corpus-2026-05-30.md`.
...
| **Session 108 endpoint (+ emit-unresolved)** | **128** | **19** |
