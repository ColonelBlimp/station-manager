...
After Session 93 closed Finding 2 + cap-sweep + hot-path audit, **only decoder-side levers remain** for the 41-signal found-but-not-decoded gap. Two candidates, with new measurement guidance:

1. **AP (a priori) decoding** — operator's preferred next-direction (revised). Per QEX paper §7. Maintains a running callsign hashtable; when an OSD candidate's c28 bits hash-match a known callsign, prior LLRs are injected at those bit positions. The Session 93 OSD CRC instrumentation directly motivates this preference: the 13 "ml-failed-but-rescuable" near-truth candidates couldn't be admitted by raw OR normalised soft-distance gating because lottery and rescue distributions overlap. AP attacks this at the root — semantic priors penalise lottery candidates because they don't match any known call. Within-slot hashtable warmup is plausible (real captures have QSO pairs that share callsigns). Significant work: hashtable accumulator + prior LLR injection + per-slot lifecycle. ~1-2 sessions.

2. **OSD-3** — drop-in extension of OSD-2 to triple-flip enumeration (C(91,3)=121,485 added candidates per OSD invocation). Cheap to implement, but the Session 93 measurement is a warning: OSD-3 multiplies the algorithmic 60% cost by 30× (corpus runtime ≈ 5+ minutes per run, making iteration painful), AND scales the same lottery-vs-rescue indistinguishability proportionally. Maybe +5-10 matched at cost of +10-20 text-extras. Worth doing as a measurement if AP work surfaces blockers, but not the recommended first move.

**Status of explored levers (post Session 93):**
...
**Performance baseline going forward:** ~19.5s for 6-slot corpus runtime, default build. `-tags osdinstr` adds ~5s to populate Finding 2 buckets when needed.

The candidate phase and the coherent-demod family are fully exhausted. Both attack surfaces are decoder-side now: enumeration depth (OSD-3) or semantic prior (AP).
