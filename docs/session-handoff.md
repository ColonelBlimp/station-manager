...
The coherent direction is largely exhausted with current tools:

- Linear coherent: do-no-harm only (Session 89).
- Piecewise: regressed (Session 91).
- **PhaseRefineFreq: shipped but unwired** — the one remaining coherent lever to measure. The slope/RMS diagnostic predicted +5 to +10 matched if it works. Cheap experiment now that the primitive exists.

**Three plausible paths in priority order (operator-flagged):**

1. **Wire PhaseRefineFreq into decode-eval and measure.** Closes the only open coherent lever. Could go either way: refinement might lift incoherent-decode rates (more candidates land on-bin → better LLRs) without needing to combine with coherent demod at all. Or it might not help if the underlying phase nonlinearity dominates.
2. **OSD-3.** 30× the OSD-2 cost; strict extra/malformed ceilings as guard. Operator earlier called this "lower priority than coherent demod first" — coherent is now exhausted, so this is back on the table.
3. **AP (a priori) decoding.** Per QEX paper §7. Significant Layer-2 + state-tracking work. Higher complexity than OSD-3.

Operator's stated preference (Session 91): post-Type-4, "if the gap is still dominated by marginal parity failures, I would try OSD-3 only as a measured experiment with strict extra/malformed ceilings. AP is likely more FT8-specific and higher implementation complexity."

Decision deferred to next session's measurement of where the 48 remaining missed decodes actually sit.
