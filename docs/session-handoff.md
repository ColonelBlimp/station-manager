...
Session 94's coherent-adaptive cancellation shipped +11 matched. The 37 remaining unmatched signals decompose into roughly:
- **7 finder misses** (Session 92's WINS_LOW + STAGE1_LOW classification — wouldn't decode even if admitted)
- **~30 decoder-bound** signals (no CRC pass after subtraction; need decoder-side levers)

**Direction options for next session, no commitments:**

1. **AP (a priori) decoding** — still the planned next direction per Session 93. Maintains a running callsign hashtable; injects prior LLRs at c28 bit positions when a candidate's hash matches a known call. Attacks both the OSD-2-unreachable cases and the lottery-indistinguishable cases. ~1-2 sessions of work. Now post-subtraction the bucket is ~30 instead of ~42.

2. **Promote coherent-adaptive subtraction to the decode-eval baseline.** Make the +11 lift the new measured-baseline for parity comparisons. Other decoder-side work (AP, OSD-3) measures relative to 107/144 instead of 96/144. Engineering: extend `decode-eval` with a `-subtract-coherent` flag (same code as unmask-probe's single-pass path).

3. **Decimation / CGO optimization of coherent-adaptive** to bring 2-iter under live budget (currently 21s/slot, want ≤15s). Plausible 3-5× further speedup; either decimation path with anti-alias + low-rate LPF (Go) or CGO/FFTW kernel ([[project_ft8_clib_fallback]] territory). Pure perf work, no parity change.

4. **OSD-3** — fallback to AP per Session 93's analysis. The session-93 lottery-indistinguishability measurement still applies (and scales worse with OSD-3's larger enumeration). Reserved as the cheaper-but-noisier alternative.

**Status of explored levers (post Session 94):**
...
- Coherent-adaptive single-pass: ~70s corpus runtime (~12s/slot), under live budget.
- Coherent-adaptive 2-iter: ~128s corpus runtime (~21s/slot), research-only.

The candidate phase and the coherent-demod family remain fully exhausted. The +11 lift came from algorithmic interference cancellation — a different attack surface from decode-eval's decoder-only pipeline.
