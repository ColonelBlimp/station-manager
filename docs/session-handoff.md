...
Two named directives from operator (Session 92 close):

1. **`stage2MaxResults` cap-sweep** ({100, 200, 500}). Cheap closer for the candidate-phase question — Session 92 sweep showed the cap is binding but the CRC-pass flatness predicts no lift. Running it makes that prediction definitive. Adds one dimension to `cmd/sweep-gates`. Expected ~3 minutes of measurement.

2. **OSD-3 or AP decoding** for the 41-signal found-but-not-decoded gap (= 144 truth − 96 matched − 7 finder-misses). The Session-92 candidate-finder sweep empirically confirmed this gap is decoder-bound. Both attack bit errors after BP convergence rather than LLR quality before. Operator's earlier preference order (Session 91) stands: OSD-3 first as a measured experiment with strict extra/malformed ceilings; AP if OSD-3 doesn't move the needle.

**Status of explored levers (post Session 92):**
...
- Candidate-finder gates: full 30-cell sweep ruled out (Session 92).

The coherent-demod family is fully exhausted; the candidate-finder phase is fully exhausted. Decoder side (post-BP) is the only remaining attack surface.
