...
- **FT8 SPA surface — holding scaffold shipped 2026-06-05; content not built.** Live FT8 decode works end-to-end but is still **log-only** (decodes are journal lines). **UI decision (supersedes the old "separate FT8 card" plan):** an **Operating Mode `<select>` inside `LoggingCard`** (header: Phone/CW ↔ FT8) toggles the body — "phone" = QsoPanel+CountryPanel+InfoPanel, "ft8" = `panels/Ft8Panel.svelte`, currently an empty holding page. `operatingMode` is in-memory `$state` (not persisted). Still to build: the live decode list (per-slot freq/DT/text) and — the architecturally-significant part — the operator workflow that turns a chosen decode into a QSO (`qsoservice` integration point ADR 0024 names; needs its own transaction-boundary decision, probably an ADR). Also noted: `ft8.device` is index-only (name-matching deferred). Parked until the operator wants live FT8 content in the browser.
