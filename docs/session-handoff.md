...
### Session 68 (2026-05-17) — FT8 milestone scope landed in `milestones.md`

Design pass, no code changes. The FT8 work that opens with ADR 0021's "scaffold only" state now has a concrete milestone breakdown so future sessions know what slice to attack.
...
- **Design preamble** captures: exact-port-first philosophy against the WSJT-X v.3.0.0.1 fork at `/home/mveary/Development/wsjtx`; **CGO committed** (FFTW3 for FFT; LDPC(174,91) bound from a C port; audio backend decided at M4.2 — portaudio leaning) — operator's call, justified by prior research showing measurable wins inside the 15s slot budget that pure-Go alternatives can't match; package boundary inherited from ADR 0021 (ft8 may consume `qsoservice`/`cat`/`serial`; mustn't import bridge; bridge mustn't import ft8 — boundary tests already in place); `jt9 -8` chosen as parity oracle for the M4.1 CI gate.
- **Defaults stated** for the four open questions so they don't block writing code: single-threaded faithful port first (concurrency only if M4.2 measurements demand it); in-memory ring buffer for decoded messages (historical "who did I hear" is logbook-app scope per the `feedback_logging_vs_logbook_scope` memory); NTP-correct host assumed with slot-offset surfaced as status (no active clock correction in v1); TX (M4.4) explicitly blocked on the inbound CAT command path landing.
- **M4.1** — WAV-file decode, parity with `jt9 -8`. Smallest end-to-end slice; CI parity gate against a bundled test corpus.
...
- ADR 0021 References section — points at `milestones.md` M4 (closes the "where do the milestones live" loop opened by the ADR).
- Memory `project_ft8_library.md` — notes that the milestone breakdown lives in `milestones.md` M4 and that CGO is committed.
...
**Next:** real FT8 content — start porting the WSJT-X v.3.0.0.1 Fortran reference at `/home/mveary/Development/wsjtx`. First decision: which sub-package to attack first (encoder? decoder? audio capture? a small leaf-utility like Costas-array generation as a warm-up?). That's a session-on-its-own conversation.
