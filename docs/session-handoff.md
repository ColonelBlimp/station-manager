...
### Session 80 (2026-05-21, late evening) — performance + sensitivity: ~28-commit day closes with 18.5× speedup and 22/48 WSJT-X-parity decodes
...
| Metric | Original | Post-session-80 | Δ |
|---|---|---|---|
| Time/op | 6.38 s | 3.5 s | 1.8× faster (sensitivity cost) |
| Memory/op | 10.06 GB | 150 MB | 67× less |
| Allocs/op | 37.9 M | 300 K | 126× fewer |
| Decode count (3 caps) | 12 (25% parity) | **22 (46% parity)** | **+83% sensitivity** |
| 15-s slot headroom | 0.4× | **4.3×** | |

**Pickup checklist for next session:**

1. Read this entry + the M4.1 progress section in `docs/v2-design/milestones.md`.
2. `git log --oneline -10` to see session 80's commit chain.
3. Sensitivity moves remaining (each session-sized chunk):
   - **OSD (Ordered Statistics Decoding)** — LDPC fallback when BP fails. Per Taylor 2020 §6, claims ~1 dB SNR gain on AWGN. ~200-300 LOC + extensive testing. Biggest remaining sensitivity prize — could close a significant chunk of the WSJT-X gap. Implementable from coding-theory references; not GPL-derivative.
   - **K-scale tuning in demod.** Per-candidate noise-floor estimate scales the LLR magnitudes so they match LDPC BP's expected magnitude range. Smaller change, harder to predict gain.
   - **Sync-step fine-timing** (re-correlate Costas at sub-step DT offsets in baseband). Now lower priority — fine-frequency recovered the VE1WT case we expected this to fix. Probably a few more decodes available but less than fine-freq turned out to give.
4. Or move on to **M4.2 live audio** — the budget concern is fully resolved (~3.5 s of 15 s slot for a hostile capture; less for typical). M4.2 wires real audio capture into the Service.Start path, replacing the placeholder goroutine.
