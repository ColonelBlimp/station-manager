...
**Resume points for next session: M3a.4** (SPA `bridge.svelte.ts` consumer + live rig test). The wire side is now feature-complete and validated against the real FTdx10. M3a.4 is purely SPA-side work:

- Replace `frontend/logging/src/lib/states/bridge.svelte.ts` stub with a real EventSource consumer.
- Three event listeners: `rig-state` (merge into `catState` — note partial-payload semantics; `*bool` SplitOverride needs a null check on the SPA side), `rig-disconnected` (toast + `bridgeState.rigResponding = false`), `bridge-error` (toast).
- `bridgeState.connected` from `EventSource.readyState`; `bridgeState.rigResponding` flips true on any `rig-state` arrival per ADR 0009.
- EventSource construction conditional on `configState.station.enabled`. Browser handles auto-reconnect.
- Vitest tests against a stub `EventSource` (jsdom).
- Live rig test: operator's actual FTdx10 connected, SPA open; dial turn → SPA's VFO display updates live; mode change → SPA reflects; power-cycle the rig → values marked stale via existing `editable` derivation, repopulate when rig comes back.

After M3a.4 lands and the operator confirms the live test, M3a (bridge subsystem v1) is closed.
