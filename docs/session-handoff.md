...
**Follow-ups noted (not blocking):** (1) go-ft8's `DecodedMessage.Sync` field returns absurd magnitudes (~1e17–1e18) — the decode text/freq/DT are correct, but `decode.go` logs `sync` as a structured field, so the daemon journal carries the junk; an out-of-tree go-ft8 question. (2) device name-matching. (3) the FT8 SPA card (separate from `LoggingCard`) is still future work — decodes are log-only, no UI yet.
