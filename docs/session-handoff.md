...
### Session 95 (2026-05-26) — LLR calibration phase 1. Robust noise estimator lifts parity 107/144 → 108/144 = 75.0%. Costas-anchor calibration path landed and measured-negative on this corpus. First crossing of the three-quarters parity threshold.
...
Sessions 94-95 shipped +12 total matched: +11 from coherent-adaptive subtraction (Session 94) plus +1 from Winsorized noise (Session 95). The 36 remaining unmatched signals decompose into roughly:
- **7 finder misses** (Session 92's WINS_LOW + STAGE1_LOW classification — wouldn't decode even if admitted)
- **~29 decoder-bound** signals (no CRC pass after subtraction + improved LLRs; need further decoder-side levers)

**Direction options for next session, no commitments:**

1. **Continue the operator's LLR-quality plan (Session 95 phase 2): per-symbol reliability weighting.** Deferred from Session 95 to land robust-noise first. Adds `reliability[i] = clamp((top1-top2)/(noise+eps), 0, max)` factor in front of each symbol's metric. Down-weights noisy rows so one bad symbol can't produce overconfident wrong bits. Could clean up the remaining edge cases.

2. **AP (a priori) decoding** — still the planned direction since Session 93. Maintains a running callsign hashtable; injects prior LLRs at c28 bit positions when a candidate's hash matches a known call. Attacks both the OSD-2-unreachable cases and the lottery-indistinguishable cases. ~1-2 sessions of work. Bucket is ~29 instead of ~42 thanks to Sessions 94-95.

3. **Promote coherent-adaptive subtraction to the decode-eval baseline.** Make the +12 lift the new measured-baseline for parity comparisons. Engineering: extend `decode-eval` with a `-subtract-coherent` flag (single-pass path from unmask-probe).

4. **Decimation / CGO optimization of coherent-adaptive** to bring 2-iter under live budget (currently 21s/slot, want ≤15s). Plausible 3-5× further speedup. Pure perf work, no parity change.

5. **OSD-3** — fallback if other directions stall. Session 93's lottery-indistinguishability measurement still applies.

**Status of explored levers (post Session 95):**
...
- **Winsorized noise estimator: +1 matched, no cost (Session 95). Loose-leaf parity win from LLR-quality side.**
...
- Baseline decode-eval (no subtraction): ~19.5s for 6-slot corpus, default build.
- Coherent-adaptive single-pass + Winsorized noise: ~70s corpus runtime (~12s/slot), under live budget.
- Coherent-adaptive 2-iter + Winsorized noise: ~128s corpus runtime (~21s/slot), research-only.
