...
The other two InfoPanel tabs (`WorkedPanel`, `SessionPanel`) are still stubs. Worked count + Session count are shown as `(0)` in the tab strip — both will be `$derived` from real state once those panels land. WorkedPanel reads recent QSOs with this callsign / DXCC; SessionPanel tracks the operator's current operating session.
...
- "Show edit history for this QSO" SPA panel — consumes `qso_history` storage from session 40. New `/v1/qso/{uuid}/history` endpoint + a panel in InfoPanel.
- "QSOs awaiting QSL request" view — now that `APP_SM_REQUEST_QSL` is being stamped on submit, the data is there. Needs a list endpoint + a worked-tab filter or a new tab.
