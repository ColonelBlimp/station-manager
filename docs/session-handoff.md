...
**Next session: M3a.2 — Serial + CAT pipeline.** ADR 0019 (bridge subsystem v1 design) and `milestones.md` M3a sub-milestones landed 2026-05-10; M3a.1 (package skeleton + config + stub SSE) shipped and review-cleanup finished same day. M3a.2 is the next concrete step: replace `runStubEmitter` with a real `internal/serial` open + `internal/cat` decode loop that publishes typed events to the existing `internal/bridge` hub. Per the M3a.2 scope in `milestones.md`: 30s data-flow timeout → `rig-disconnected`, field filter (vfoA/vfoB/mode/subMode/selectedVfo/splitOverride/power; drop waterfall/S-meter), stub-rig harness for hardware-free integration tests.
...
| `internal/bridge/` | ✅ M3a.1 shipped (skeleton + config + SSE + stub emitter + hub + boundary tests) | M3a.2 replaces stub with real pipeline |
