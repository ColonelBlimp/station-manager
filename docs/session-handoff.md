...
### Session 126 (2026-06-04) — Inbound rig-command path: data-driven `cat` rework + `bridge.SendCommand` + `POST /v1/rig/command` + `BridgeInfo.ops`. Daemon-side **shipped + tested**; ADR 0026 revised; HW validation + SPA pending. Implementation committed (`5e8af9b7`); `BridgeInfo.ops` + this doc pass pending commit.

Picked up the Session-125 pivot and built the whole daemon-side inbound path (set_freq/set_mode). ADR 0026 stays **Proposed**; its Decision/Consequences were revised to match the shape below.
...
**Pending (per ADR 0026):** (1) **validate confirm-by-push on the real FTdx10** — the load-bearing unproven assumption: `curl -X POST .../v1/rig/command -d '{"op":"set_freq","value":14074000}'` → expect 202 + the rig jumps + the new freq returns over `/v1/rig/events` with no extra read. If it doesn't volunteer state, the explicit-re-read fallback is needed. (2) `ft8.bands` config block. (3) SPA FT8 card (band buttons compose set_freq+set_mode; freq nudge = client arithmetic over set_freq). (4) SPA i18n catalogue entries for the three new HTTP codes.
