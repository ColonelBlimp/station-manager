...
- **FT8 SPA surface not built.** As of session 121 (2026-06-02) live FT8 decode works end-to-end but is **log-only** — decodes land as journal lines, with no UI. The eventual surface is a dedicated FT8 card (separate from `LoggingCard` per the cards-vs-panels convention): a live decode list (per-slot, with freq/DT/text), and — the architecturally-significant part — the operator workflow that turns a chosen decode into a QSO (the legitimate `qsoservice` integration point ADR 0024 names; needs its own transaction-boundary decision, probably an ADR). Also noted: `ft8.device` is index-only (name-matching deferred). (The go-ft8 `Sync` junk-value issue was fixed upstream in v0.1.1, session 122.) Parked until the operator wants live FT8 in the browser, not just the logs.
