...
Clean-install environment: reinstated 2026-09-06 as a fresh deployment on the dogfood host itself (package,
working directory, unit and linger removed by `task reset:local:dev` after a verified copy of the alpha.2
working directory; the lane on the rows reads **live host, fresh**). Earlier: retired 2026-09-05 by operator
risk decision; the partially prepared
...
repository; remote archive SHA-256 recorded at A3-01. A fresh full local copy of the working directory
was taken by the operator on 2026-09-05 with `smd` stopped (Execution log #11); its location is retained
...
NOT EXECUTED by operator risk acceptance at the time; recovery is the stopped-daemon working-directory copy or
ADIF import). Superseded in part 2026-09-06: the clean-install rows were executed on the dogfood host itself and
ruled PASS (A1-02, A2-01–A2-07); the rollback rehearsal stays retired
Operator/date: 7Q5MLV, 2026-09-05

Dogfood accepted: pending
Operator/date:
Residual waivers: B1-01 WAIVED 2026-09-06 (Finding #6, recoverable config refusal on the alpha.1-normalised
qrzcq filter; population bounded to this already-corrected station; expires at the next candidate) — the row
carries the full statement
Follow-up destinations: W-0008 CC-5 (Finding #6 fix, backlog P1 #1) and CC-6 (`config-check` parity,
Finding #1); W-0011 (Finding #7, P1 promotion conditional on passive reproduction); W-0012 (Findings
#3, #4, #8, #11, #12, #16); W-0018 (Findings #9, #10, #13–#15); W-0009 (Finding #17); W-0010 (SM
Cloud https on the LAN); W-0002 (VU24DX sighting); `install.md` §7 (file-only keys across an
upgrade, Findings #1/#5)

## Scope reduction — operator decision, 2026-09-05

**Amended 2026-09-06:** the operator reinstated the clean-install rows A1-02 and A2-01–A2-07 for alpha.2 as a
fresh deployment on the dogfood host (not nspawn): a verified byte-identical copy of the alpha.2 working
directory was taken first (the return path), then `task reset:local:dev` removed the package, unit, linger
and working directory. The frozen RPM (SHA unchanged) is installed per the guide; the evidence belongs to
alpha.2 only — the next candidate earns its own. The nspawn lane, failure injection and PKG-10a/10b stay
retired.
...
- Lane labels (the first token of every row's environment cell): **live** = the daily-driver station
  ...
  **nspawn** = retired lane; every such row is NOT EXECUTED under the scope decision above (the clean-install
  rows were later executed on the host under the **live host, fresh** lane, 2026-09-06);
...
(received 2026-09-05, Execution log #11).
...
- **`config.json` version 2 → 3** (`currentConfigVersion` 2 → 3, `3c45fc48`): `migrateV2toV3` consumes retired
  v2 keys so an upgraded install boots, and normalisation rewrites the file with current defaults on the first
  start (observed at B1-04: +20 key paths, Finding #2). The earlier statement here that no config migration was
  added was wrong — corrected 2026-09-06.
...
- **Unknown `config.json` keys are rejected before any startup write** (`3c45fc48`, `24febadc`;
  ...
  ignored on load — and dropped on its next write (Finding #5) — now refuses to start with a named key. Exercised deliberately and in isolation at
...
|---|---|---|---|---|---|---|---|---|
| `A1-01` | yes | **record** — Clean tree at `b8e0356f`, tag `v2.0.0-alpha.2` | Build once with `task rpm:dev:pocketfft`; record filename, SHA-256, RPM version | One artifact; RPM version `2.0.0~alpha.2-1`; SHA recorded above | A `-dirty` or `alpha.1-N-g…` version (built from an untagged or dirty tree) | header fields; step-1 recheck 2026-09-05 11:10Z: HEAD `b8e0356f` = tag `v2.0.0-alpha.2^{}`; both RPM copies `7b2fa20a…c24856`, 9,053,968 B; installed `2.0.0~alpha.1.1238.gd9cd38ae.dirty-1`; `smd` inactive, no process; tag local only; `-dirty` in `git describe` is the uncommitted capsule/record only | none | PASS — operator ruling 2026-09-05 |
| `A1-02` | yes | **live host, fresh** — the A2-01 install on the dogfood host, candidate installed and started | Verify the candidate's SHA-256 before install; then compare `GET /v1/version`, the sidebar build badge, the startup log line and `rpm -q station-manager` | All four read `2.0.0-alpha.2` (RPM `2.0.0~alpha.2-1`); SHA matches the header | Badge shows a cached previous build; log shows `dev`; a different SHA (a rebuilt or overwritten `…-dev` RPM) | 2026-09-06: SHA `7b2fa20a…c24856` verified before install; `GET /v1/version` `2.0.0-alpha.2`; startup log records stamped `2.0.0-alpha.2`; `rpm -q` `2.0.0~alpha.2-1`; sidebar badge read alpha.2 — operator, Firefox, 11:18Z | none | PASS — operator ruling 2026-09-06 |
| `A1-03` | yes | **record** — Record drafted | Review the release delta above against the candidate (migrations, configuration, journeys, integrations, manual/setup) and correct anything missing | Delta complete enough that every B case has a cause | Accepting the delta without opening `git log d9cd38ae..v2.0.0-alpha.2` | reviewer initials | none | PASS — operator ruling 2026-09-05; the delta was corrected through the operator's review rounds (manual path, migrations, configuration) before ratification |
| `A1-04` | yes | **host-scratch** — a 0700 scratch directory holding a copy of the live `config.json`; the candidate `smd` extracted from the frozen RPM (Appendix 2); live service untouched | Run the read-only preflight `smd config-check --config` on the untouched copy (control), then on the two mutated copies (a top-level and a nested unknown key) | Control exits 0 ("no unrecognised keys"); each mutation exits non-zero naming only the key path; nothing started, bound or written | Control exits non-zero — the candidate would refuse to start on the station: fix the live config and re-run (**no waiver is possible**); a mutation exits 0 (old behaviour); a value printed | run 2026-09-05 11:15Z (Execution log #4): SHA verified; `smd` extracted, never installed; control on the live copy → exit 0 "no unrecognised keys"; top-level `typo_key_b1_05` → exit 1 naming only the path; nested `bridge.typo_nested_b1_05` → exit 1 naming only the path; live `config.json` mtime unchanged (2026-08-18 14:01:28); `smd` inactive before/after; scratch shredded and removed | none | PASS — operator ruling 2026-09-05 |
| `A2-01` | yes | **live host, fresh** — dogfood host after `task reset:local:dev` (baseline 11:02Z: package/binary/unit/enablement absent, linger off, no working directory, port free, RPM SHA unchanged; leftover `~/.local/share/station-manager.new` from the pass-1 revert noted, not consulted by the pinned unit) | Install per `install.md`; enable/start user service; note lingering step | Service active; health and version endpoints answer; logs readable; browser reaches `/` | Service starts but binds loopback only and the guide's URL differs; lingering omitted so the service dies at logout | 2026-09-06 13:05 CAT (Execution log #23): `sudo dnf install` of the frozen RPM, `daemon-reload`, `enable --now`, `loginctl enable-linger` per the guide; `rpm -q` `2.0.0~alpha.2-1`, `rpm -V` clean; unit `enabled`, `active (running)`, `NRestarts=0`, linger `yes`; listener `127.0.0.1:8080` only; `/v1/version` `2.0.0-alpha.2` schema 8 clean, `/v1/healthz` ok, `GET /` 200, `setup_complete: false`; working directory created with `config.json` 0600 (version 3, three default forwarder entries, no rigs), `db/` 0700, empty database (0 QSOs, 0 logbooks); first-start log: migrations, `HTTP server listening`, zero errors or warnings; lingering evidenced by state (`Linger=yes`, unit enabled), not by a logout | none | PASS — operator ruling 2026-09-06 |
| `A2-02` | yes | **live host, fresh** — first browser open after A2-01 | Open the app | Only the first-run welcome surface; no operate/logbook chrome | Dashboard placeholder renders before setup completes | operator screenshot 11:10Z (Firefox, `http://localhost:8080`): the "Welcome to Station Manager" card with the callsign field and Save only — no sidebar, no operate/logbook chrome; API `setup_complete: false` at the same time. Tab title read "Dashboard · Station Manager" on the welcome surface — Finding #8 (operator: bug) | none | PASS — operator ruling 2026-09-06 |
| `A2-03` | yes | **live host, fresh** — first-run callsign form | Enter invalid, empty, lowercase/whitespace-normalized, then valid callsign | Each state explains itself; the normalized form is shown before acceptance | Lowercase accepted silently without showing the normalized value | operator, Firefox, screenshots 11:16Z: `123456` → Save refused with the "Invalid callsign format" error toast; empty field → Save button disabled; `  7q5mlv  ` → the field shows the normalised `7Q5MLV` before Save is pressed; valid callsign then saved | none | PASS — operator ruling 2026-09-06 |
| `A2-04` | yes | **live host, fresh** — valid callsign submitted | Complete setup | Default logbook created; both **Open Settings** and **Start logging** offered | Only one journey offered, or logbook missing in Logbook view | operator screenshot 11:17Z: "Setup complete — your default logbook is ready" with both **Open Settings** and **Start logging** offered; server side at the same time: `setup_complete: true`, default logbook id 1 `7Q5MLV` present in the database, 0 QSOs, config rewritten 0600 | none | PASS — operator ruling 2026-09-06 |
| `A2-05` | yes | **live host, fresh** — setup complete | Reload browser; restart daemon; reopen | Setup stays complete; no welcome surface | Welcome reappears after restart (setup flag not persisted) | operator: after "Start logging" and a tab reload the app opened without the welcome surface; then `systemctl --user restart smd` at 13:20:05 CAT — journal `Stopping`/`Stopped`/`Started`, new main PID, daemon log `smd stopped` → `smd starting` → `HTTP server listening` in the same second — and the reopened app again showed no welcome (11:20Z); server side `setup_complete: true`, default logbook 1, version `2.0.0-alpha.2` schema 8, zero errors or warnings in the fresh log | none | PASS — operator ruling 2026-09-06 |
| `A2-06` | yes | **live host, fresh** — setup complete, guide and manual only | Find Settings, the embedded Manual, and log a first QSO using only the install guide + manual | All three reachable without repository knowledge; note every gap | Reaching Manual only via a URL learned from the repo | operator, guide and manual only, 11:35Z: Settings reached from the sidebar; the embedded Manual reached from the sidebar (opens in the same tab — Finding #4); first QSO on Operate → Phone/CW needed the undocumented no-rig band confirmation (Finding #9) and a manual mode change after the band button left FT8 selected (Finding #11); the install guide's rig section is stale (Finding #10). Logged QSO verified read-only: id 1, `7Q7EB`, 20m, SSB, 14.200 MHz, 59/59, 1 row; no upload rows (no forwarder enabled); zero errors | none | PASS — operator ruling 2026-09-06 |
| `A2-07` | yes | **live host, fresh** — installed, four QSOs in the test logbook | Uninstall per the guide | Package removed; operator data deliberately retained as documented | Data directory deleted, or package left half-removed | operator ran the guide's §10 steps (11:58Z): `disable --now`, `sudo dnf remove`, `daemon-reload`, `disable-linger`; verified: package not installed, `/usr/bin/smd` and `smctl` gone, unit file gone, no enablement link, linger off, no process, port free; `~/.local/share/station-manager` retained with `config.json` 0600 and the database (4 QSOs, schema 8) exactly as §10 documents; residue: two empty unowned directories under `/usr/share/doc/station-manager` (Finding #17) | none | PASS — operator ruling 2026-09-06 |
| `A3-01` | yes | **live** — Dogfood station, daemon stopped or quiescent | Back up the complete working directory outside the repo | Backup exists; listable | Backup copied into the repository tree | local staging archive 2026-09-05 11:14Z (Execution log #3): `~/sm-backup/station-manager-pre-alpha2-20260905T1114Z.tar.gz`, 18,185,501 B, mode 0600, SHA-256 `066f769b38914ba8fed3cc31f7082d5b9f76ae6d3e169390dccab2f0cd0503b9`, 67 entries, listable; `smd` quiescent and the log database unheld at archive time (WAL present and included); operator copied it with the prescribed 0700-directory/0600-archive sequence to the private SMC LAN host (exact endpoint retained outside the repository); remote SHA-256 matched exactly | none | PASS — operator ruling 2026-09-05 (local archive and remote copy, hashes equal) |
| `A3-02` | yes | **host-scratch** — restore of the A3 archive: Backup made | Restore a copy into a scratch location; open it read-only | Restore completes; record counts readable | Backup lists but does not restore (permissions / partial copy) | run 2026-09-05 11:18Z (Execution log #5): archive extracted into a 0700 scratch dir (68 entries); database opened read-only (`mode=ro`): `schema_migrations_log` `(7, 0)`, `integrity_check` ok, 7,468 `qso` rows (7,468 with `deleted_at` NULL; logbook 1), `qso_upload` 13,089 `uploaded` + 1 `failed` (forwarder_type `qrz`), `operator_event` absent — all equal to A3-03; scratch config copies shredded, directory removed; live untouched | none | PASS — operator ruling 2026-09-05 |
| `A3-03` | yes | **live** — Pre-upgrade | Record durable-record counts/checksums (QSOs per logbook, upload-queue rows by status, notifications) | Numbers recorded here without contents | Counting after the upgrade began | Read-only count, 2026-09-05: logbook id 1 — 7,468 total/active QSOs, 0 deleted; upload queue — 13,089 uploaded, 1 failed; `operator_event` absent as expected before migration 0008 | none | PASS — operator ruling 2026-09-05; the live log database's SHA-256 equalled the archived copy at ruling time (Execution log #11), so the counts still hold |
| `A3-04` | yes | **live** — pre-upgrade | Produce and verify all three rollback artifacts: the reconstructed alpha.1 RPM (`rpmrebuild`; NVR and SHA-256 recorded), the A3 archive, and the rehearsed database rollback path from PKG-10a/10b (Appendix 3) | Artifacts present beside each other; the header names the rehearsed path (down-migration, with the workdir-restore variant), its quiescence barrier and its post-upgrade-QSO handling | Only the RPM preserved — alpha.1 refuses the version-8 database and the rollback stalls mid-window | reconstructed RPM 2026-09-05 11:40Z (Execution log #6): `build/private/x86_64/station-manager-2.0.0~alpha.1.1238.gd9cd38ae.dirty-1.x86_64.rpm`, 7,359,884 B, mode 0600, SHA-256 `c4fd092f329980c32283e773e0311ab07e7ae5f621ff177362b88e9ca174a807`; exact NVR/arch and payload size/mtime/digest/mode/owner/group match; scripts/triggers/file capabilities match; regenerated-header differences recorded in the log; local/remote A3 archive hashes match; PKG-10a/10b still pending | none | PASS (reduced scope) — operator ruling 2026-09-05: the reconstructed alpha.1 RPM and the A3 archive are verified; the rehearsed rollback path was retired by the scope decision and is not claimed |
| `A3-05` | yes | **live** — Before starting B1 | Confirm rollback time is reserved before the station is next needed | Window recorded in "Operator parameters" | Upgrading right before an operating session | note | none | NOT EXECUTED — the reserved rollback window was retired with the formal rehearsal by the operator's 2026-09-05 scope decision |
...
| `SETUP-06` | yes | **nspawn** — Settings → Forwarding | Edit a forwarder incl. a credential; save; reload | Credential redacted on read and preserved on save; https requirement enforced for credentialed forwarders | Masked secret written back as the mask string | screenshots + forwarder still works | none | NOT EXECUTED — scope retired by operator risk acceptance (2026-09-05) |
| `SETUP-07` | yes | **nspawn** — Settings → Email | Edit SMTP settings incl. password; save; send a test if offered | Redaction/preservation as SETUP-06; rejection text not leaked into logs | Password cleared on an unrelated save | screenshots + log grep | none | NOT EXECUTED — scope retired by operator risk acceptance (2026-09-05) |
| `SETUP-08` | yes | **nspawn** — Settings → Enrichment | Edit provider settings/credentials; save; reload | As SETUP-06 | — | screenshots | none | NOT EXECUTED — scope retired by operator risk acceptance (2026-09-05) |
...
| `SETUP-11` | yes | **nspawn** — Settings save with the daemon paused | Save, then resume the daemon | Outcome reported as unknown with edits kept; re-read reconciles (F-04c) | "Failed" toast while the save actually persisted | screenshot + config on disk | none | NOT EXECUTED — scope retired by operator risk acceptance (2026-09-05) |
...
| `PHONE-07` | yes | **nspawn** — Same station again within a minute | Submit a duplicate | Duplicate handled as documented (fold or explicit deliberate-repeat) | Second contact silently lost | Logbook | none | NOT EXECUTED — scope retired by operator risk acceptance (2026-09-05) |
| `PHONE-08` | yes | **nspawn** — Submit while the daemon is paused | Submit, resume | Unknown-outcome wording; re-read shows the QSO once (F-04a) | Retry creates a second QSO | Logbook | none | NOT EXECUTED — scope retired by operator risk acceptance (2026-09-05) |
...
| `PHONE-16` | yes | **nspawn** — Provoke a visible error (e.g. forwarder credentials rejected) | Observe recovery guidance | Error names the action the operator can take; logging stays usable | Generic "error" with no recovery | screenshot | none | NOT EXECUTED — scope retired by operator risk acceptance (2026-09-05) |
...
| `LOG-04` | yes | **nspawn** — A QSO row | Change a field and save; enter an invalid value | Changed edit persists; validation refuses | Invalid value stored | row history | none | NOT EXECUTED — scope retired by operator risk acceptance (2026-09-05) |
| `LOG-05` | yes | **nspawn** — A QSO row, daemon paused | Save an edit, resume | Unknown wording, re-read confirms (F-04a) | Duplicate revision on retry | row history | none | NOT EXECUTED — scope retired by operator risk acceptance (2026-09-05) |
...
| `LOG-10a` | yes | **live host, fresh** — reinstated 2026-09-06, default logbook | ADIF import into the default logbook | Duplicate/error summary; restart handoff documented | Import replays after an unverified rollback | 11:47Z (Execution log #24): guide §5 `smctl import` stored the three synthetic records into the default logbook (id 1) with band/mode/date/time/RST preserved; 0 duplicates, 0 errors, no uploads | none | PASS — operator ruling 2026-09-06 |
| `LOG-10b` | yes | **live host, fresh** — next fresh deployment, an instance with more than one logbook | ADIF import into an explicitly selected (non-default) logbook | Duplicate/error summary; restart handoff documented | Import replays after an unverified rollback | not exercisable on the single-logbook 2026-09-06 instance; a supported product path, so not N/A | none | pending (next fresh deployment) |
| `LOG-11` | yes | **nspawn** — Scratch logbook | Export, then re-import the export | Round-trip clean; counts equal | Re-import creates duplicates | counts | none | NOT EXECUTED — scope retired by operator risk acceptance (2026-09-05) |
...
| `MAP-06` | yes | **nspawn** — Force a data error (e.g. daemon returns an error for the window) | Observe | Error state named; map remains usable | Empty map shown as "no contacts" | screenshot | none | NOT EXECUTED — scope retired by operator risk acceptance (2026-09-05) |
...
| `PKG-03` | yes | **live host, fresh** — reinstated 2026-09-06 (guide §5, recommended next step); synthetic 3-record ADIF in the throwaway instance | `smctl import` journey | Documented output; summary matches LOG-10 | Import handoff leaves the daemon stopped | operator ran `smctl import ~/sm-backup/synthetic-history.adi` exactly as the guide shows (11:47Z): "Stopping SM for import (exclusive database access)… SM Stopped.", `3/3 (stored=3, duplicate=0, errors=0)`, "uploads: none (use --forward to queue a forwarder)", "Restarting SM… SM Started."; verified read-only: 4 QSOs (the logged one plus the three imported, fields intact), 0 upload rows, daemon restarted 13:47:02 CAT, health ok, zero errors | none | PASS — operator ruling 2026-09-06 |
| `PKG-04a` | yes | **nspawn** — `smtest` session with linger enabled per the guide | Exit the `machinectl shell` (logout), check from the host, then `machinectl reboot fedora44` and check again (Appendix 4) | Daemon survives logout and is up after the container boots without a manual start | Needs a manual start after either | transcript | none | NOT EXECUTED — scope retired by operator risk acceptance (2026-09-05) |
...
| ID | Required | Environment and starting state | Operator action | Expected visible result | Nearest confusable failure | Evidence | Extra approval | Result |
|---|---|---|---|---|---|---|---|---|
| `DOCS-01` | yes | **nspawn** — Clean-install operator (A2-06) | Follow the guide to a first loggable QSO | Reachable; gaps noted | Guide references the retired config SPA | notes | none | NOT EXECUTED — scope retired by operator risk acceptance (2026-09-05) |
...
11. **16:55Z — Gate A rulings and the operator's fresh local copy.** Operator direction: close the
    retired rows and rule the evidenced rows PASS. Read-only rechecks immediately before ruling:
    candidate RPM SHA-256 `7b2fa20a…c24856`, 9,053,968 bytes; installed
    `2.0.0~alpha.1.1238.gd9cd38ae.dirty-1`, `smd` inactive, no process; no file under
    `~/.local/share/station-manager` newer than the 11:14Z archive; the log database, its WAL, the
    reference database, `evidence.db` and `config.json` all SHA-256-equal to their archived copies;
    the candidate NVR sorts after the installed one (`rpm.labelCompare` = 1), so `rpm -Uvh` upgrades
    without `--oldpackage`. The operator reported a full copy of the working directory taken with
    `smd` stopped; its location is retained outside the repository. Rulings: A1-01, A1-03, A1-04,
    A3-01, A3-02, A3-03, A3-04 (reduced scope), A4-01 and A5-01 PASS; A5-02 N/A; A1-02, A2-01–A2-07,
    every B2 row whose lane was nspawn, PKG-10a, PKG-10b and A3-05 (the retired window) NOT EXECUTED
...
17. **18:10Z–18:12Z — B1-06 controlled restart (operator-run).** `systemctl --user restart smd` at
    ...
    the operator's.
18. **18:19Z–18:22Z — revert, operator-run: daemon stopped and working directory restored.** No QSO
    was logged during pass 1 (the operator confirmed; count still 7,468). The operator stopped
    alpha.2 at 20:19:27 CAT and restored the working directory from the stopped-daemon copy.
    Verified read-only: `smd` inactive, no process, log database not held, port 8080 free; every
    regular file in the restored directory is SHA-256-identical to the A3 archive (67 entries, no
    extra files), the only difference being the mtime of `station-manager.db-shm`, touched by the
    read-only inspection itself; `schema_migrations_log` `(7, 0)`; `config.json` version 2, 6,919
    bytes, mtime 2026-08-18, `allow_insecure_http` unset. Package still `2.0.0~alpha.2-1` — the
    alpha.1 reinstall is the next step; reconstructed alpha.1 RPM SHA-256 re-verified
    `c4fd092f…4a807`, NVR `2.0.0~alpha.1.1238.gd9cd38ae.dirty-1`. The pass-1 working directory's
    location after the operator's restore is not recorded here (nothing in it needs preserving).
19. **18:23Z–18:24Z — alpha.1 reinstalled and running on the restored directory; acknowledgement
    dropped.** Operator-run: `allow_insecure_http` added to the restored config, `sudo rpm -Uvh
    --oldpackage` of the reconstructed alpha.1 RPM, `daemon-reload`, `start` at 20:23:16 CAT.
    Verified read-only: `rpm -q` = `2.0.0~alpha.1.1238.gd9cd38ae.dirty-1`; `/v1/version`
    `2.0.0-alpha.1-1238-gd9cd38ae-dirty`, schema 7 clean; `qso` 7,468, uploads 13,089 uploaded / 1
    failed; three forwarder workers started, HTTP listening, zero errors. But alpha.1 rewrote
    `config.json` at start (mtime 20:23:16, 7,060 bytes) — it normalised the qrzcq forwarder's
    `action_filter`/`batch_size`/`tick_interval_sec` — and, because that build ignores unknown keys
    on load and re-marshals its typed struct on every write (`config.go` `UnknownKeys` comment and
    `WriteJSON` at `d9cd38ae`), the file-only `allow_insecure_http` was dropped: now unset (Finding
    #5). alpha.1 has no config file watcher; it re-reads only on a signal and writes only on a
    settings save, so a key added now, after the startup write, survives until the pass-2 restart
    provided nothing is saved. Plan: operator re-adds the key while alpha.1 runs; presence and mtime
    re-checked immediately before the `dnf install` and again immediately before the restart.
20. **18:27Z–18:31Z — pass 2: swap under the running daemon OK, candidate refused the normalised
    config.** After the operator re-added `allow_insecure_http` (present, mtime 20:24:56; a second
    save at 20:25:54 left size and key unchanged), `sudo dnf install` replaced the package under
    running alpha.1 — `rpm -q` `2.0.0~alpha.2-1`, alpha.1 pid unchanged serving
    `2.0.0-alpha.1-1238-gd9cd38ae-dirty`, `/proc/<pid>/exe` marked deleted, no journal event, new
    unit on disk — exactly the row's expectation. `daemon-reload` + `restart` at 20:29:04: alpha.1
    logged `shutdown signal received` → `HTTP server draining` → `HTTP server shutdown complete` →
    `smd stopped` within the second; alpha.2 then exited status 1 five times before any daemon-log
    record, stderr (journal): `invalid config (invalid_forwarder): forwarder "qrzcq": type "qrzcq"
    does not support action "update" (supports [insert])`; systemd start limit, unit failed, no
    process. Database untouched: schema `(7, 0)`, `operator_event` absent, counts equal. Config
    unchanged since 20:25:54, `allow_insecure_http` still true, qrzcq `action_filter` `[insert,
    update, delete]` (written by alpha.1's startup normalisation, Execution log #19). The
    candidate's read-only `config-check` on a copy still exits 0. The open page shows CAT red /
    daemon unavailable, which is correct while no daemon runs. Finding #6. Nothing changed by me; no
    hardware or RF.
21. **18:34Z–18:36Z — pass 2 completed after the operator's qrzcq filter edit.** Operator set the
    qrzcq `action_filter` to `["insert"]`, `reset-failed` and started at 20:34:45 CAT: `active
    (running)`, `NRestarts=0`. Startup on the restored schema-7 database: `databases open and
    migrated` → `schema_migrations_log` `(8, 0)`, `operator_event` present (0 rows); config migrated
    v2 → v3 (7,508 bytes) keeping `allow_insecure_http: true` and the `[insert]` filter; three
    forwarder workers, reconciler, HTTP listening 20:34:48; the open page's streams re-subscribed at
    20:34:49. Endpoints: version `2.0.0-alpha.2` schema 8, health ok, setup complete, forwarders
    unchanged, queues smcloud 0/0; `mode=ro` counts equal A3-03 (7,468; 1,695 / 2,010+1 / 9,384).
    Listener loopback only. **Observation:** at 20:34:48, three seconds after the CAT port opened,
    the bridge logged `TX ALARM — the rig may still be transmitting; check the radio` (error) and in
    the same second `tx alarm cleared (transmitter confirmed idle)`; no durable notification row
    resulted; no such record at the 19:49 or 20:10 starts. Whether the FTdx10 was keyed by the
    operator at that instant is the operator's to say; if not, it is a startup transient to log for
    triage. No rig command, recheck, keying or RF was initiated by me.
22. **2026-09-06 — findings triaged, Finding #6 waived.** The operator ratified the triage of
    Findings #1–#7 and chose to waive B1-01 for this candidate rather than re-freeze: the failure
    was recoverable, touched no data, and its population is bounded to this already-corrected
    station; the waiver expires at the next candidate, which must carry the narrow W-0008 CC-5
    reconciliation of the alpha.1-generated qrzcq filter shape (explicit unsupported actions stay
    rejected). Finding #7 goes to W-0011 with a stated acceptance outcome and a conditional P1
    promotion; Findings #3/#4 to W-0012; the SM Cloud https decision to W-0010; the VU24DX sighting
    to W-0002; both delta corrections applied above; `install.md` §7 gains the file-only-key
    guidance. The five 2026-09-05 inbox notes are routed and removed from the inbox. All other
    pending rows remain pending.
23. **11:02Z–11:06Z — fresh deployment on the host: baseline and A2-01.** Operator decision: run the
    clean-install rows on the dogfood host rather than the container. Return path first: the
    operator's copy `~/sm-backup/station-manager-alpha2-current` verified byte-identical to the live
    working directory (66 entries, zero content differences, sensitive files 0600; schema 8, 7,468
    QSOs, 13,089 uploaded / 1 failed, config v3 with `allow_insecure_http` and the `["insert"]`
    filter). `task reset:local:dev` (operator, sudo) then removed the package, binary, unit file,
    enablement links, linger and working directory; baseline at 11:02Z clean except the leftover
    `~/.local/share/station-manager.new` (the pass-1 directory moved aside on 2026-09-05; holds
    credentials; not consulted by the unit's pinned path; operator to remove or archive). A2-01 at
    13:05 CAT: install per the guide; every check clean (row evidence). Live daemon for the station
    is therefore replaced by the fresh instance for the duration of A2; the alpha.2 copy is restored
    afterwards.
24. **11:06Z–11:58Z — fresh deployment on the host: A2-02 to A2-07, §5 and §6 of the guide.** With
    the live daemon replaced by a fresh install of the frozen RPM: A2-02 welcome only; A2-03 all
    four callsign states; A2-04 setup complete with both journeys; A2-05 setup persisted across a
    reload and an evidenced daemon restart; A1-02 badge alpha.2; A2-06 Settings and Manual
    reachable, first QSO logged from the documentation alone but only after the undocumented no-rig
    band confirmation; guide §5 `smctl import` of a 3-record synthetic ADIF (PKG-03, LOG-10 default
    half) stored 3 / 0 uploads and the Worked panel showed the imported contact; guide §6 file
    locations all correct; A2-07 uninstall left the data directory as documented. Findings #8–#17
    raised on the way (tab title, no-rig confirmation undocumented, stale rig-editor text, band
    button leaves FT8 mode, right rail collapsed by default, manual CAT heading, manual not
    releasable, no offline uninstall docs, stale header count after import, unowned doc directories)
    — none a retention or data failure. Station now: package removed, test data directory present,
    return copy intact; the return path (delete test directory, reinstall the frozen RPM, restore
    the alpha.2 copy, start) follows.
25. **11:58Z–12:01Z — return to the real station.** Operator: removed the throwaway test directory,
    `sudo dnf install` of the frozen RPM, `cp -a` of `~/sm-backup/station-manager-alpha2-current`
    into place, `daemon-reload`, `enable --now`, `enable-linger`. Verified read-only at 14:00 CAT:
    `rpm -q` `2.0.0~alpha.2-1`, `rpm -V` clean, unit enabled and active, linger yes, loopback
    listener; `/v1/version` `2.0.0-alpha.2` schema 8 clean, health ok, setup complete, four
    forwarders as before; `mode=ro` counts 7,468 QSOs, clublog 1,695 / qrz 2,010+1 failed / smcloud
    9,384 uploaded; config version 3 with `allow_insecure_http: true` and the `["insert"]` qrzcq
    filter, mode 0600; every regular file content-identical to the copy (log and SQLite WAL/SHM
    excluded); startup clean with only the three expected warnings and no TX alarm at this bridge
    open. The fresh-deployment excursion is closed; the station is the pre-excursion alpha.2 state
    plus nothing.
26. **12:14Z — rulings applied and Findings #8–#17 routed.** Operator rulings 2026-09-06: PASS for
    A1-02, A2-01–A2-07, A5-03, B1-02–B1-06 (B1-04 on its migration/count outcome after remediation,
    B1-05 against alpha.2's key-only preflight contract) and PKG-03; B1-01 stays WAIVED as recorded;
    LOG-10 split into LOG-10a (default logbook, PASS) and LOG-10b (explicitly selected logbook,
    pending the next fresh deployment — a supported path, not N/A). Findings routed: #8, #11, #12,
    #16 → W-0012; #17 → W-0009; #9, #10, #13, #15 → the new W-0018 "Bring the embedded manual to
    release readiness" (Finding #14), placed after W-0012 in P2 as a page-by-page standing pass
    during dogfooding and a release gate before a public release. The ten routed inbox notes were
    removed; the startup-failure-visibility note remains untriaged. Findings #1 and #6 now cite the
    shipped CC-6 and CC-5 precisely. Cleanup, operator-run and verified 12:16Z: the leftover pass-1
    directory `~/.local/share/station-manager.new` removed permanently; the `fedora44` container
    image removed; the `/etc/systemd/nspawn/fedora44.nspawn` override removed; `machinectl
    list-images` reports none. `machinectl remove` was blocked by SELinux, so the operator performed
    a bounded direct removal of the image directory instead. Host-side check: no image, no override,
    no `/var/lib/machines/fedora44`, no `.new` directory. No Station Manager product finding arises
    from the cleanup.
27. **08:39Z — station moved to dev builds; Finding #4 fix deployed and accepted.** Operator
    decision: deploy and test each W-0012 fix on the station. `f0b8e6eb` (Manual link in a new tab)
    was committed, then `task deploy:local:dev` built and installed `2.0.0~alpha.2.14.gf0b8e6eb-1`;
    verified: active, `/v1/version` `2.0.0-alpha.2-14-gf0b8e6eb`, schema 8, 7,468 QSOs, health ok,
    zero errors; the operator confirmed the Manual opens in a new tab. From this point the station
    runs dev builds past the frozen candidate: B2 rows walked from now on belong to the next
    candidate's record, and the return path to alpha.2 (frozen RPM plus the 2026-09-06 copy) goes
    stale for QSOs logged on dev builds.
28. **08:39Z–09:07Z — Finding #8 fix deployed, accepted on a first-run card, station swapped back.**
    `c1045efa` (Welcome tab title) was deployed as `2.0.0~alpha.2.16.gc1045efa-1`. To see a welcome
    card on the running build the operator moved the real working directory aside to
    `station-manager.orig` (11:01 CAT), let the daemon write a fresh first-run directory (11:03,
    `first run: wrote default config to disk`), read the title, then stopped the daemon, removed the
    fresh directory and moved the real one back. Verified after the swap: `/v1/version`
...
30. **12:37Z–12:38Z — initial-comment change deployed; the count request now lands at the
    reconnect.** `9b60b65e` (every SSE stream sends `: connected` as its first body bytes) deployed
    as `2.0.0~alpha.2.21.g9b60b65e-1`. The deploy's own restart served as the acceptance restart
    with the page open and untouched: streams disconnected 14:37:06 CAT, daemon started 14:37:12,
    `/v1/events` subscriber connected 14:37:16 and `/v1/logbook/1/count` requested in the same
    second, with no `/v1/config` beside it (the reconnection path, not a page load); the rig
    stream's `/v1/version` re-fetch also at 14:37:16. Compared with entry 29's thirty seconds, the
    count refresh is now immediate. A later disconnect/reconnect at 14:37:27–14:37:35 carried a
    `/v1/config` fetch and was a page reload, not a daemon restart. A direct probe of the live
    stream returned `: connected` followed by a blank line as its first thirteen bytes. Zero errors.
    Finding #16 is closed as deployed and verified.
31. **04:17Z–06:52Z — Finding #3 fix deployed and accepted.** `a364c21a` (the default rig cannot
    be deleted: Delete is disabled on it with the reason stated in the panel and the tooltip, the
    store refuses locally and again on its fresh re-fetch, and the earlier silent repoint of
    `default_rig_id` to the first survivor is gone) was deployed as `2.0.0~alpha.2.23.ga364c21a-1`,
    daemon started 06:17:17 CAT. Verified: `/v1/version` `2.0.0-alpha.2-23-ga364c21a`, schema 8,
    health ok, 7,468 QSOs, zero errors in the daemon log since the restart. The operator reported
    the fix deployed and accepted against the verification list (Settings → Rigs, three rigs
    configured: the default rig's Delete greyed with the reason shown, another rig's Delete live,
...
33. **14:28Z–14:31Z — Finding #3's tooltip-only follow-up deployed and accepted.** `17fc6a7e`
    removes the "The default rig can't be deleted — set another rig as default first" line under
    the rig detail header; the Delete button's `title` alone carries the reason, and its
    disabled/enabled logic is unchanged (the Finding 3 pin now asserts the panel line is absent
    and the tooltip present). Deployed as `2.0.0~alpha.2.29.g17fc6a7e-1`, daemon started 16:28:52
    CAT; verified `/v1/version` `2.0.0-alpha.2-29-g17fc6a7e`, schema 8, health ok, 7,468 QSOs,
...
|---|---|---|---|---|
| 1 | B1-04 (pass 1) | The candidate exits at startup: the SM Cloud forwarder's `credentials.url` is cleartext http to a non-loopback host and `allow_insecure_http` is unset, so `smcloud.New` refuses (`securehttp.CheckCredentialedURL`, ST-4a, `cf269984`); the orchestrator fails the `workers` start and the process exits 1; systemd's start limit stops the loop after five tries. The delta named the policy ("SM Cloud LAN cleartext needs an explicit acknowledgement") but nobody mapped it onto this station's config, and `smd config-check` validates keys, not forwarder construction. | working-as-designed (ST-4a), unmapped at A1-03; station remediated by the operator's `allow_insecure_http` decision; the preflight gap is real: `config-check` validates keys only | W-0010: move SM Cloud to https even on the LAN (operator decision 2026-09-05); W-0008 CC-6 (shipped `6ddb4c97`): `config-check` runs `config.Load` plus the enabled-forwarder Build gate — exactly that, not the whole daemon start — so a policy refusal surfaces at A1-04/B1-05 |
| 2 | B1-04 (pass 1) | Before failing, the candidate migrated the live `config.json` from `version` 2 to 3 (`currentConfigVersion` 2 → 3 between `d9cd38ae` and the candidate, `3c45fc48`) and rewrote it: +20 key paths (forwarder 3 overrides `action_filter`/`batch_size`/`endpoints.insert`/`tick_interval_sec`, `lookup.chain.*.priority`, a second `lookup.chain` entry with `url`/`view_url`/`username`/`password`/`timeout_sec`/`enabled`/`name`, `lookup.continue_if_blank`), size 6,919 → 7,472 bytes, mtime now 2026-09-05 19:40 CAT; a new 10-byte `pskreporter.id` appeared. The delta's "No config.json version migration was added" is wrong. | documentation correction; the v2 → v3 rewrite is expected behaviour | record: delta Migrations bullet corrected 2026-09-06 |
| 3 | B1-03 (pass 1, Settings → Rigs) | The default rig profile shows an enabled Delete button (not pressed). Operator expectation: the default rig must not be deletable; the default must be changed first. Code, read-only: the SPA disables Delete only for the only rig (`RigsSection.svelte:42`, `rigs.svelte.ts:560`); deleting the default rig is deliberate — `deleteRig` repoints `default_rig_id` to the first surviving rig because the daemon refuses an unresolvable default (`config.Validate` re-checked under the config lock, `handler_config.go:758`), so no dangling-default or data-loss path exists; the delete is confirm-dialog-guarded and has no undo. | working-as-designed mechanically (auto-repoint, server validates); UX-policy change ratified | W-0012 — shipped `a364c21a` 2026-09-08 (Delete disabled on the default rig with the reason in the panel and the tooltip; the store refuses locally and on its fresh re-fetch; the auto-repoint removed); deployed as `2.0.0~alpha.2.23.ga364c21a-1`, accepted 2026-09-09 (Execution log #31); the panel line removed by ruling in `17fc6a7e`, deployed as `2.0.0~alpha.2.29.g17fc6a7e-1`, accepted 2026-09-09 (Execution log #33) |
| 4 | B1-03 (pass 1, sidebar) | The sidebar Manual link (`Sidebar.svelte:169`, `href="/manual/"`) opens in the same tab, navigating away from the app; the operator expects a new tab. The sidebar already opens another link with `target="_blank"`, so the pattern exists in the file. | UX defect, low severity; seen again at A2-06 on the fresh deployment 2026-09-06 (the sidebar item even carries an external-link glyph) | W-0012 — shipped `f0b8e6eb` 2026-09-07 (`target="_blank" rel="noopener"`, pinned); operator verified on the station's dev build `2.0.0-alpha.2-14-gf0b8e6eb` |
| 5 | revert (Execution log #19) | The old daemon (alpha.1, `d9cd38ae`) ignores unknown `config.json` keys on load and drops them on its next write — its startup normalisation rewrote the file and removed the `allow_insecure_http` acknowledgement the operator had added to the restored config. The delta's wording that the old daemon "silently kept" unknown keys is therefore only true until its next write. Operational consequence: a file-only key needed by the candidate must be added after the old daemon's last write, immediately before the upgrade restart. | documentation correction plus operator guidance | record: delta Configuration bullet corrected 2026-09-06; `install.md` §7: add file-only keys for the new version after the old daemon's last write |
| 6 | B1-01 (pass 2) | Upgrade over the running alpha.1 fails at the restart: alpha.2 exits before opening the databases with `invalid config (invalid_forwarder): forwarder "qrzcq": type "qrzcq" does not support action "update" (supports [insert])`. Cause: alpha.1's startup normalisation fills an omitted `action_filter` with the historical all-three default for a type that registered no supported set (`config.go:1227` at `d9cd38ae`), so the disabled qrzcq forwarder now carries `[insert, update, delete]`; the candidate registers qrzcq as insert-only and `validateForwarders` rejects the filter (`config.go:1843`). Any alpha.1 install whose config the old daemon has written with a qrzcq forwarder present, enabled or not, cannot start alpha.2 without a hand edit. `smd config-check` passed this file (unknown keys only) minutes before. | **defect, upgrade-blocking** for alpha.1 → alpha.2 stations with a qrzcq entry; recoverable (no data touched, key named) → B1-01 WAIVED for this candidate. Ratified fix is NARROW (shipped as CC-5, `db4bcb58`): in the v2→v3 migration, a pre-version-3 document whose `qrzcq` forwarder carries exactly the ordered slice `["insert","update","delete"]` alpha.1's omitted-filter path emitted becomes the literal `["insert"]`; no reconciliation-specific record — covered by the existing one-time schema-migration record; every other explicit unsupported action, including a permutation or the same content in a version-3 document, stays rejected (`RegisterSupportedActions` contract, `registry.go:185`; `TestValidateForwarders_RejectsUnsupportedAction`, `forwarder_actions_test.go:49`). Tests must prove both: the legacy shape loads, an operator-authored `[insert, update]` still fails | W-0008 CC-5, backlog P1 #1; ships in the next candidate; waiver on B1-01 expires there |
| 7 | B1-04 (pass 2 start) | Three seconds after the bridge opened the FTdx10 CAT port at the 20:34:45 start, it logged `TX ALARM — the rig may still be transmitting; check the radio` (error) and, in the same second, `tx alarm cleared (transmitter confirmed idle)`; the operator confirms nothing was transmitting; no durable notification resulted; not seen at the 19:49 or 20:10 starts. | defect on the safety surface: false TX alarm at bridge open, cleared within the second; acceptance outcome — opening the CAT port with an idle rig never raises a TX alarm, while a rig genuinely keyed at open still raises one within the existing latency; no settle duration or re-read mechanism chosen yet | W-0011 now; promotion to backlog P1 #2 only once passively reproduced at port open (operator agreement per occasion, receive only) |
| 8 | A2-02 (fresh deployment) | On the first-run welcome surface the browser tab title reads "Dashboard · Station Manager" (operator screenshot 2026-09-06 11:10Z). | UX defect (operator ruling 2026-09-06) | W-0012 — shipped `c1045efa` 2026-09-07 (shared `setupGateOpen` predicate titles the card "Welcome"; pinned by mounting App); deployed as `2.0.0~alpha.2.16.gc1045efa-1` and accepted |
| 9 | A2-06 (fresh deployment) | With no rig configured, logging the first QSO is blocked by "Rig not confirmed — confirm the band in the Rig panel before you can log this QSO" (Log QSO disabled) until the header Rig panel's **confirm** control is used. Neither the install guide nor the embedded manual mentions the no-rig band-confirmation step or that control (repository search 2026-09-06: no hit in `manual/content` or `docs/install.md`). | first-run documentation defect (operator ruling 2026-09-06) | W-0018 plus install guide §4 |
| 10 | A2-06 (fresh deployment) | `docs/install.md` §4 "Connecting a rig (CAT)" says "There is no rig editor in the SPA yet (that's the future config app), so this is a `config.json` edit" — stale since the config SPA was folded into the app; Settings → Rigs exists. | documentation correction (operator ruling 2026-09-06) | W-0018 |
| 11 | A2-06 (fresh deployment, no rig) | Clicking **20m** in the Rig Control card opened from Operate → Phone/CW set the frequency to 14.200.000 (from 14.074.000) but left the mode on FT8 (operator screenshot 11:33Z: card "Manual — confirm to log", 20m selected, Mode FT8, 14.200.000). Operator expectation: the card knows whether it was opened from Phone/CW or FT8 and sets both frequency and mode for that context (USB here). | UX defect — no-rig operating-context policy (operator ruling 2026-09-06) | W-0012 (not W-0011) |
| 12 | A2-06 (fresh deployment) | On a new install the left-hand navigation rail defaults to expanded while the right-hand rail on the Operate page defaults to its minimum width (operator screenshot 2026-09-06 11:36Z, first session after setup). Operator expectation: both rails default to expanded. | working-as-designed (operator ruling 2026-09-07, closing the 2026-09-06 UX-default ruling): both rails default to expanded when browser storage is empty (`initialNav`/`initialUtil` return `full`); a rail is narrow only from the stored `sm-nav`/`sm-util` preference or below the 61rem/72rem width thresholds. The screenshots rule out the threshold (the rail toggled open at the same window size), so the collapsed right rail was Firefox's remembered `sm-util: narrow` for the `http://localhost:8080` origin, which survives a reinstall because localStorage belongs to the browser, not the daemon | closed; no product change. Any future "reset UI preferences per daemon instance" behaviour would be a W-0012 design decision, not a fix |
...
| 19 | B2 (dev build `2.0.0-alpha.2-54-g94506823`, Settings → Forwarding, 2026-09-12) | The QRZ card reads "1 queued · 0 in flight" while every recent QSO, including both of the morning's, is on QRZ. Read-only DB inspection: the one row is `qso_upload` 32804, a `failed` qrz insert for QSO 7025 (7Q7EB, 2026-08-06 06:32Z, 20 m SSB), one attempt at 06:33Z, `last_error` = QRZ's "invalid api key" response, no retry scheduled; QRZ uploads had succeeded until 05:15Z that day and resumed at 13:20Z (488 since, the latest 2026-09-12 04:20Z). Three defects: (a) the card counts `clearable` = pending + failed as "queued" (tooltip "waiting to send"), so a dead row reads as a live backlog and nothing on the card names the QSO or the reason; (b) an authentication rejection is terminal by design (`qrz.go`), but once the key was corrected nothing re-armed the row — the only recovery is a Logbook re-send or a queue clear, the W-0010 outcome-1 class (a rotated credential stranding rows) shown to be forwarder-agnostic; (c) the failure predates durable notification history (W-0001, late August), so it left no trace the operator could find; and the stored error text echoes the rejected key verbatim (inbox note 2026-09-12; the value is stale, not the current key). Not to be confused with the operator's 7Q7EB test entry of this morning: today's `smd.log` has no line for that call and no QSO was stored for it — the queued row is QSO 7025 from 2026-08-06, which the log shows stored on `2.0.0-alpha.1-1107-g9c012341` 11 s after that fresh build's HTTP server came up, 3 s after the callsign was committed, and about 50 s before the FT8 stream first opened — consistent with a test QSO logged during that morning's FT8-seed dogfooding — the operator confirmed 2026-09-12 it was a UI test, not a contact; it is live in the Default logbook and was mirrored to SM Cloud (never to QRZ or ClubLog). Removal: delete it from the Logbook (soft-delete plus a delete-forward to every enabled forwarder whose filter includes delete), then clear the QRZ queue, since a delete leaves the failed insert row in place and the delete-forward itself fails terminal at QRZ for want of a prior `upstream_id` (a second failed row; W-0010 outcome 9 names the no-op fix); ClubLog's response to a delete-by-fields for a QSO it never held is unverified. | defect (operator ruling 2026-09-12) | W-0010 — outcome 9 (split failed from waiting on the card and name the rows; re-arm auth-failed rows when that forwarder's credential changes, or offer "retry failed"; redact the echoed key in `qrz.classifyResponse`) |
