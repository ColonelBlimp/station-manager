...
- **[ADR 0008](../decisions/0008-notifications-toast-system.md)** — Notifications/toast system: hand-rolled `$state`-array singleton at `lib/states/toasts.svelte.ts`; `<Toasts/>` mounted once at `top-right` (revised 2026-05-03 to match v1 convention); three levels (info/warn/error) with TTL 4/6/8s; max-stack 5; click-to-dismiss; toasts express *events*, inline messages express *state*.
- **[ADR 0009](../decisions/0009-cat-state-decomposition.md)** — CAT-state decomposition (refines [ADR 0006](../decisions/0006-cat-state-precedence-rule.md) structurally): four state objects with static ownership — `catState` (rig mirror, bridge-only writes), `manualState` (operator edits in CAT-off mode), `configState` (operator-declared station properties), `displayedState` (derived view that components read). Power-with-amp-multiplier is the canonical example: `displayedState.effectivePower = configState.station.ampEnabled ? catState.power × configState.station.ampMultiplier : catState.power` (the toggle was added session 36, 2026-05-05; was previously unconditional multiplication). `split` and `selectedVfo` become structurally derived projections — no writeable `split` field anywhere.
...
- `TX_PWR` is the *effective* radiated power per `displayedState.effectivePower` — `rawPower * ampMultiplier` when `configState.station.ampEnabled` is true, raw rig power otherwise. Rounded to integer. Omitted when 0 ("not set").
...
|---|---|---|---|
| Daemon (`/v1/config`) | Operator declarations / cross-device persistent | `configState.{setupComplete, loggingStation, defaultLogbook, defaultRig}`, plus the daemon-persisted amp pair `configState.station.{ampEnabled, ampMultiplier}` (added session 36, 2026-05-05; backed by `types.StationConfig`) — wired via `lib/api/config.ts` `fetchConfig()` / `putConfig()` (built session 31, 2026-05-03). `configState.station.enabled` (CAT-enabled) stays SPA-only on this tier — not round-tripped through `/v1/config`. | Persistent on the daemon's filesystem (`config.json`, atomically rewritten by `cfgSvc.Update()`); SPA boots via `app.svelte` `onMount` → `fetchConfig()` and PUTs back via the setup form / My Station panel's Update button |
| `localStorage` (cont.) | Per-device operator preferences for QSO emission | `qsoDefaults.qsoRandom` (`'Y'` / `'N'` / `'off'`) under `sm.qsoDefaults.qsoRandom` (added session 36) | Survives reload + tab close; not daemon-persisted by deliberate scope choice |
...
- `configState.station.enabled`, `.ampEnabled`, and `.ampMultiplier` use `$state` because `displayedState.isLive` and `.effectivePower` derive from them.
...
- ~~**Toast / notifications system.**~~ **Resolved 2026-05-01 in [ADR 0008](../decisions/0008-notifications-toast-system.md); built 2026-05-03.** Hand-rolled `$state`-array singleton at `lib/states/toasts.svelte.ts` exposing `pushToast`/`dismissToast` plus `toasts.info/warn/error` shortcuts. `<Toasts/>` mounted once at `app.svelte` shell as `top-right` fixed (revised from ADR's original bottom-right to match v1's convention); click-to-dismiss; per-level auto-dismiss (info=4s/warn=6s/error=8s); `ttl=0` opts out for sticky. Max-stack=5 with oldest-evicted semantics. Tailwind cluster `.toast-base/.toast-info/.toast-warn/.toast-error` in `app.css @layer components`. **Severity prefix:** `<strong>Info: </strong>` / `Warning: ` / `Error: ` rendered by `Toasts.svelte` from `toast.level`, not by callers — single source of truth + accessibility. First consumer: `QsoPanel.submitQso()` — duplicate=warn, validation/server/network=error. CAT-handover toast (per ADR 0006) is the next planned consumer.
