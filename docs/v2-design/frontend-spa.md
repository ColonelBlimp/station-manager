...
- **No new daemon endpoints.** The SPA consumes the existing `/v1/*` surface; new endpoints get added when the SPA actually needs them. The logging form's POST `/v1/qso` already exists; the enrichment endpoint `GET /v1/enrich/callsign` (ADR 0017) shipped 2026-05-07 — the SPA's `lib/enrichment.svelte.ts` thin fetch wrapper is the next deferred piece.
...
- ~~**Enrichment pipeline shape.**~~ **Resolved 2026-05-01 in [ADR 0005](../decisions/0005-enrichment-pipeline-shape.md), refined 2026-05-07 in [ADR 0017](../decisions/0017-enrichment-pipeline-domain-table-cache.md) (which supersedes 0005)**: single daemon endpoint `GET /v1/enrich/callsign?call=X`, aggregated JSON response, always-200 contract, AbortController cancellation. ADR 0017's refinements (vs the original 0005 framing): hamnut is country source-of-truth (not a peer to QRZ); domain tables (`country` + `contacted_station`) are the cache, not a dedicated cache table; callsign-class providers form a sequential priority chain (not concurrent fan-out); always-merge filter→hamnut at return time. Daemon-side shipped 2026-05-07. SPA's `lib/enrichment.svelte.ts` becomes a thin (~30 line) fetch + abort-handle wrapper — pending. Reverses the prior "SPA orchestrates concurrent hamnut+QRZ fetches" framing — incompatible with [ADR 0004](../decisions/0004-daemon-vs-spa-responsibilities.md)'s rule that external-service orchestration lives daemon-side. See [`enrichment.md`](enrichment.md) for operational detail.
