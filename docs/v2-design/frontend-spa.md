...
| `localStorage` (cont.) | Operator's comment paste-list (reusable canned phrases) | `commentHistory.items` (string[], newest-first, deduped, capped at `MAX_COMMENTS`=10) under `sm.commentHistory.items` as a JSON array. Populated automatically on each `case 'stored'` submit; offered back via the Comment field's ▾ dropdown. | Survives reload + tab close; per-device |
...
- **`lib/ui/components/`** — form primitives. Callsign, VfoBox, Rst, etc. `Comment` carries an optional paste-list: pass a non-empty `history` (newest-first) + `onpick` and a ▾ trigger on the label row opens a popover of recent comments (click to fill). QsoPanel wires it to `commentHistory.items` (replace-on-pick) and feeds it via `commentHistory.add()` on each saved QSO. Escape closes the popover and `stopPropagation`s so the form-level Escape (clear QSO) doesn't also fire.
