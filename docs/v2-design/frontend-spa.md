...
- ~~**`station` store migration into `configState.loggingStation`.**~~ **Done 2026-05-05 (sessions 34-35).** All ADIF MY_* identity fields now live on `configState.loggingStation` as plain class fields (per the reactivity audit). `MyStationPanel.svelte` surfaces the operator-typed set (identity → location → equipment → CW); `MyLat`/`MyLon` are daemon-derived from `MyGridsquare` via `internal/utils/maidenhead.go`'s deterministic Maidenhead → ADIF "XDDD MM.MMM" math, hydrated read-only into the panel. `QsoPanel.submitQso` sources every MY_* tag from `configState` and `formatAdifRecord` emits the full bucket. Zones (`MyCqZone`, `MyITUZone`) and `MyDXCC` stay operator-typed for v1 — daemon-side polygon-driven derivation was deliberately out of scope (no bundled dataset). The legacy `lib/stores/station.ts` and the empty `lib/stores/` directory were deleted. Per-QSO bearing math (`AntAz`) lives in `lib/utils/bearing.ts` and is wired to format on submit; country-panel display of short/long path is the next hookup work.
