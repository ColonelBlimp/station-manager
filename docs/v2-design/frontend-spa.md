...
- ~~**`station` store migration into `configState.loggingStation`.**~~ **Done 2026-05-05 (sessions 34-36).** All ADIF MY_* identity fields now live on `configState.loggingStation` as plain class fields (per the reactivity audit). `MyStationPanel.svelte` surfaces the operator-typed set as **four sub-tabs** (identity / location / equipment / CW) using the same `tab-item`/`tab-button` pattern as the parent `InfoPanel` strip — no icons + smaller font marks the visual nesting, `activeSection` is panel-local `$state`. An **Update button** at the bottom of the panel (outside the tab bodies so it persists across section switches) PUTs the full `logging_station` block via `lib/api/config.ts`'s `putConfig(...)` and re-applies the daemon's response: canonical normalisations (callsign upper-case, gridsquare mixed case) and derived fields (`MyLat`/`MyLon`) flow back into the UI without a manual refresh. Info-toast on success, error-toasts for validation/server/network outcomes (mirrors the setup-dialog pattern in `app.svelte`). `MyLat`/`MyLon` are daemon-derived from `MyGridsquare` via `internal/utils/maidenhead.go`'s deterministic Maidenhead → ADIF "XDDD MM.MMM" math, hydrated read-only into the panel; the SPA never sends them on PUT. `QsoPanel.submitQso` sources every MY_* tag from `configState` and `formatAdifRecord` emits the full bucket. Zones (`MyCqZone`, `MyITUZone`) and `MyDXCC` stay operator-typed for v1 — daemon-side polygon-driven derivation was deliberately out of scope (no bundled dataset). The legacy `lib/stores/station.ts` and the empty `lib/stores/` directory were deleted. Per-QSO bearing math (`AntAz`) lives in `lib/utils/bearing.ts` and is wired to format on submit; country-panel display of short/long path is the next hookup work.
