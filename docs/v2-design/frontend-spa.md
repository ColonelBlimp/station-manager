...
The shape is settled. The daemon `POST /v1/qso` endpoint accepts this exact body. The SPA's network wrapper is `lib/api/qso.ts` (added 2026-05-03) — `submitQso(adif: string, logbookID: number): Promise<SubmitOutcome>` returns a discriminated union (`stored` / `duplicate` / `validation` / `server` / `network`) so `QsoPanel` branches on outcome without parsing JSON envelopes inline. Logbook id is hardcoded to `1` until a logbook switcher lands.
