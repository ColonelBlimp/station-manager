...
| In-memory only | Live transport / rig state | `catState`, `bridgeState`, `qsoDraft` (state singleton; lifted out of QsoPanel session 33), `station` store contents | Rebuilt from rig events / app boot |
...
- **`station` store migration into `configState.loggingStation`.** `lib/stores/station.ts` carries hardcoded ADIF MY_* defaults (`stationCallsign`, `gridSquare`, `name`, `rig`, `antenna`) that the QSO submit path reads directly. Session 33 caught `STATION_CALLSIGN` mismatching the daemon's logbook validator because the My Station card writes to `configState.loggingStation.stationCallsign` while submit was still reading the store's hardcoded `'G4ABC'`. Patched the one field; the rest still ship via the store. **Migration plan:** extend `/v1/config` to surface the remaining MY_* fields, add them to `configState.loggingStation` (plain class fields per the reactivity audit — no $derived consumers), wire My Station panel to write them through `ValidatedInput`, switch `submitQso` to source ALL identity fields from `configState`, delete `lib/stores/station.ts`. Must precede any new daemon-side validator on those fields — the same trap will recur otherwise.
