...
>
> **Implementation status (2026-05-10):** M3a.1 + M3a.2 + M3a.3 shipped (skeleton, real serial+CAT pipeline, bootstrap-on-SSE-open via rigdef-driven READ, bridge-error events with hub cache for late subscribers, identity verification, multi-subscriber fan-out). Wire side feature-complete and validated against real FTdx10 via `cmd/catcli`. M3a.4 (SPA `bridge.svelte.ts` consumer + live rig test) is the next milestone — see `milestones.md` §M3a.4. After that lands, M3a closes.
