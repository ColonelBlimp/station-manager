...
| 2026-05-10 | INIT/READ wire-shape split for M3a.3 bootstrap-on-SSE-open | Bridge subsystem M3a.3 needed a per-SSE-open snapshot poll (per ADR 0019) so a fresh SPA tab sees real rig state without waiting for the operator to wiggle the dial. Two clean roles separated: `INIT = "AI1;"` (arms AUTO push state — sent once at pipeline startup, silent on wire); `READ = "ID;FA;FB;ST;VS;MD0;MD1;PC;"` (full identity + state snapshot — sent per SSE-open via `Service.TriggerBootstrap`). Pre-fix `INIT = "AI1;ID;"` and `READ = "FA;FB;ST;VS;MD0;MD1;PC;"` overlapped on the identity query and the snapshot didn't include identity (so late SPA tabs missed `RigIdentity`). New shape verified live on the operator's FTdx10 via `catcli -init -read -listen` — all 8 expected decodes flow. Both shipping rigdefs (FT-710, FTDX10) updated; encode fixtures repinned. |
| 2026-05-10 | `cmd/catcli` gained `-read` flag | Companion to `-init`. After init fires, sends the rigdef's READ command and lets the framed responses flow through `-listen`. Live-test command for verifying any rigdef's M3a.3-style wire pattern: `catcli -device /dev/ttyUSB0 -rig <id> -init -read -listen` produces 8 decoded snapshot lines plus subsequent AUTO pushes. |
...
- Extended with `-init` flag: when set with `-rig`, sends the rig's `INIT` command (via `cat.Encode`) at startup. For Yaesu rigs this enables AI push-state mode (`AI1;` — silent on wire, no response expected post-M3a.3).
- Extended with `-read` flag (M3a.3, 2026-05-10): when set with `-rig`, sends the rig's `READ` command after any `-init`. For Yaesu rigs that's `ID;FA;FB;ST;VS;MD0;MD1;PC;` — 8 framed responses decoded by cat. Mirrors the bridge's per-SSE-open bootstrap pattern; useful for live-testing a new rigdef's wire shape before the bridge integration.
...
- **Rig-specific mode → ADIF translation — SHIPPED 2026-05-11 (session 51).** The translation layer the M3a.4 live test surfaced as a gap is in place. **Architecture as settled:** the bridge stays pure pass-through (emits the rig's literal mode string on the wire — no mapping logic added there). SPA resolves the rig string to an ADIF (MODE, SUBMODE) pair via a per-rig table stored in two layers: rigdef-shipped defaults inside each `internal/cat/rigs/*.json` under `mode_mappings`, and operator overrides under `bridge.mode_mappings` in `config.json`. Daemon merges the two at `/v1/config` GET time (operator's value wins per rig-string key on collision); SPA receives the merged view, the `displayedState.mode` / `.subMode` derivations consult the table when CAT is live. **Open questions resolved as follows:** (1) operator override — new My Station → Mode Mappings sub-tab lists one row per rig mode (from the rigdef's MAINMODE value_mappings), two free-text inputs for ADIF MODE and SUBMODE; PUT to daemon stores deviations from the rigdef default as overrides; operator who runs FT8 most of the time can leave DATA-U → FT8 default; one who switches to PSK31 edits + saves; (2) CW-U vs CW-L — both collapse to `MODE=CW SUBMODE=""` in the shipped defaults (ADIF doesn't refine CW upper-vs-lower sideband; rare operator preference can override); (3) per-rig vs shared — per-rig in the rigdef JSON because the mode-string vocabulary IS rig-specific; cross-rig sharing isn't a real concern given different rigs literally produce different strings. **Bonus side-effect:** the daemon's `internal/enums/modes` hardcoded ADIF enum was replaced by a data-driven catalogue — embedded `adif-modes.json` baseline + optional operator-side `$SM_WORKING_DIR/modes.json` override — so future ADIF spec growth doesn't strictly require a code change (operator can extend the override file ahead of a daemon binary update).
