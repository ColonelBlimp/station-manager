...
- **Rig-specific mode → ADIF translation (parked 2026-05-11, session 50).** Surfaced during M3a.4 live testing: the FTdx10 reports mode strings (`DATA-U`, `DATA-L`, `CW-U`, `CW-L`, `RTTY-U`, `RTTY-L`, `FM-N`, `AM-N`, `DATA-FM`, `DATA-FM-N`, `PSK`) that do not line up 1:1 with ADIF's MODE+SUBMODE pair. ADIF main modes the daemon accepts: `AM / CW / FM / RTTY / SSB / DIGITALVOICE / MFSK / PSK / HELL / PACKET`; `DATA-U` has no canonical ADIF equivalent — semantically it's "USB tuning, digital signal expected" but the actual protocol (FT8, RTTY-D, PSK31, etc.) is decided by the running software, not the rig. Today the SPA's mode `<select>` widget shows blank when CAT pushes a value outside its 9-entry dropdown list, and submitting a QSO with `MODE=DATA-U` would 400 against the daemon's strict ADIF enum. **Decision (operator, 2026-05-11):** build a rig-specific translation layer — per-rig "what the rig CAT gives us → what that represents in the ADIF spec"; **defer the design to a follow-up session.** Likely shape: extend each `internal/cat/rigs/*.json` driver with a `mode_to_adif` table mapping the rig's raw mode strings to `(ADIF MODE, ADIF SUBMODE)` pairs; bridge looks the table up before emitting `rig-state` so the SPA only ever sees ADIF-valid pairs. Open questions for that session: where does the operator override the protocol when the rig reports a generic `DATA-U` and they're actually running FT8 (a writable dropdown alongside the read-only CAT display? a "current protocol" toggle outside the QSO form?); whether `CW-U` vs `CW-L` should carry through as a SUBMODE refinement or collapse to plain `CW`; whether the table belongs in the rigdef JSON (per-rig) or a shared CAT-mode catalogue (per-family). Until that lands the SPA's mode field is best-effort: it shows whatever the rig pushes (and blanks the dropdown when the rig pushes an unknown string), and the operator selects the closest ADIF submode in the dropdown when CAT is off.
