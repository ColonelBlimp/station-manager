...
| `active_qso_archive_id` / `pending_qso_archive_id` | `string` | Server-managed selectors into `qso_archives` (ADR 0071): the archive the daemon serves, and — only while an activation is in flight — the candidate for the next start. Empty before adoption. At start the daemon opens the pending candidate when one is set, otherwise the active archive; the candidate becomes active (pending cleared, its `last_activation_error` cleared, written file-first) only once every database-dependent node is up and before the HTTP server serves. A candidate that does not come up — file missing, another archive's identity, a failed node, or a promotion that cannot be written — is torn down, its failure recorded in `last_activation_error` and pending cleared (memory first; an unwritable `config.json` keeps the request on disk for the next start), and the daemon starts once more on the active archive. Commands (`smd import`, `smd restore`) always resolve the active archive. |
| `qso_archives` | `[]types.QsoArchiveConfig` | Server-managed station-global archive catalogue: `id` (immutable UUIDv7, also embedded in the file's `archive_metadata` row, which is the authority), `label` (mutable, never renames the file), `ownership` (`managed` — path derives from the id; `legacy` — the adopted pre-archive file at its recorded absolute `path`; `external` — operator-owned file at its absolute `path`), `last_activation_error` (holds the STABLE failure code of the most recent failed activation — `archive_file_missing`, `archive_identity_mismatch`, … — never raw error text or a path; the API words it), `request_key` (the creation request's idempotency key, v5). Written by adoption at start and the archive manager only; `PUT /v1/config` carries no field for it. Managed archives live at `<data_dir>/db/qso-archives/<id>.db` (directory `0700`, files `0600`); the manager builds a new one as `<id>.db.creating`, migrates it with the log set, seeds its logbook and identity, checks integrity, closes it to a single file, renames it into place and only then adds the inactive entry — a `.creating` file left by an interrupted creation is named in the log at every start, never listed as an archive and never deleted by the daemon. |
...
Version `4 -> 5` (W-0021 slice 2C) adds `qso_archives[].request_key`, the creation
request's idempotency key, so a retried create — through a restarted daemon too —
