...
| `FieldDay.{Class,Section}` | D (TX, deferred) | served; PUT-writable (presence-aware) | — | Operator's ARRL Field Day exchange for answering CQ FD (search & pounce). Empty = unset. Class strict (`^[1-9][0-9]?[A-F]$`, in `types`); Section checked against go-ft8's canonical ARRL/RAC list (`ValidARRLFieldDaySection`, in `internal/config` — `types` stays stdlib-only). Stored upper-cased |
