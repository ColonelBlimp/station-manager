...
- **Response:** **200** `{"enabled": bool, "state": "disabled"|"capturing"|"drop_new", "cap_bytes": int, "watermark_bytes": int, "usage_bytes": int, "observations": int, "unprofiled_observations": int, "dropped_slots": int, "profiles": {...}, "sync": {...}}`. `usage_bytes` is physical (evidence.db + WAL + shm); `drop_new` means usage crossed the soft watermark and new capture is being counted as loss intervals rather than written; `unprofiled_observations` is the §5.4 missing-profile tally.
