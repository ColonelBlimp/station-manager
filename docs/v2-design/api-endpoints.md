...
- **Gating:** Always-on.
- **Request:** Body = `ConfigResponse` shape. Writable: `logging_station`, `station`, `ft8_display` (presence-aware — omit to leave untouched), `bridge.mode_mappings` (diffed against rigdef defaults; only deviations stored, on the active rig). `setup_complete`, `mailer`, `ft8_frequencies` are server-managed/ignored.
