...
- **Response:** **200** SSE stream. Events:
  - `ft8-occupancy` → `OccupancyReport` `{slot: {start_utc, period}, passband: Band, dial_mhz?, signal_width_hz, occupied: [Band], suggested: [int]}` (`Band` = `{low_hz, high_hz, source?, level?}`). `dial_mhz` is the rig dial the slot was CAPTURED on (absent when the daemon has no CAT to read it from). As with `ft8-qso`'s `dial_freq_mhz`, clients must attribute the report to a band from THIS, never from live rig state: publication lags capture by the decode, so a report in flight across a band change would otherwise be labelled with the new band and its `suggested` offsets — measured elsewhere — could steer a transmission. A slot whose dial moved mid-window is not published at all. Not replay-cached (see `internal/ft8/hub.go`).
