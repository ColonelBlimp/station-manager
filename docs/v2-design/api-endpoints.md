...
- **Purpose:** Begin a sequenced Call-CQ session that works answerers (ADR 0033). Answerer-selection mode = daemon config `ft8.tx.caller_answer_mode` (default `auto_first`; `auto_strongest`; `operator_pick` per ADR 0065 — answerers are LISTED on the `ft8-qso` frames (`answerers`) instead of auto-committed, the CQ keeps calling until the operator pops one via `POST /v1/ft8/cq/pick`, and the run works that station then resumes CQ).
- **Gating:** **Only when FT8 is enabled.** Requires TX armed.
