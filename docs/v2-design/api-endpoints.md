...
- **Request:** Path `{name}` (a destination **binding name** of the active archive — on the adopted archive the seeded legacy config names; an additional logbook's binding is `<type>.<logbook uuid>`). Body `{"uuids": ["…", …], "force": false}`. `uuids` are deduplicated; `force` (default false) re-sends QSOs already uploaded to this destination instead of skipping them.
- **Behaviour:** action is always `insert`. Each UUID lands in exactly one bucket — enqueued, `skipped_uploaded` (already on this destination, no force), `skipped_deleted` (soft-deleted; not backfilled), `skipped_other_logbook` (the QSO belongs to a logbook other than the one the named binding serves — a binding uploads its own logbook's QSOs only, ADR 0082), or `not_found` (unknown/malformed). Per-QSO best-effort: one bad UUID never fails the rest. "Already uploaded" = an existing insert-upload row at status `uploaded` for this forwarder (keyed by name, so N-agnostic across forwarder types).
- **Response:** **200**, body `{"enqueued": N, "skipped_uploaded": M, "skipped_deleted"?: ["…"], "skipped_other_logbook"?: ["…"], "not_found"?: ["…"]}`.
- **Errors:** 400 `invalid_forwarder` (empty name); 400 `missing_required_field` (empty `uuids`); 400 `batch_too_large` (> 5000 uuids); 400 `forwarder_unavailable` (no **enabled binding** of that name forwards inserts in the active archive — a disabled binding has no worker and gets its queue rows discarded at startup, so enqueuing would strand them; ADR 0082); 400 malformed body; 500 `enqueue_failed`.
...
- **Gating:** Always-on.
- **Request:** No body.
- **Behaviour:** One entry per **binding of the active archive** (enabled or disabled, resolved or not, in the archive's listing order — the seed's config order on the adopted archive). `waiting` = rows at status `pending` (still to be sent); `failed` = rows at status `failed` (terminal until re-armed — read apart from `waiting` since W-0010 outcome 9, so a failure never shows as a live backlog); `clearable` = `waiting + failed` (what a clear removes); `in_flight` = rows at status `in_progress` (the currently-claimed batch, never cleared); `uploaded` history is counted in none. A forwarder with no queued rows reads all zeros.
- **Response:** **200**, body `{"forwarders": [{"name": "…", "waiting": N, "failed": N, "clearable": N, "in_flight": N}, …]}`. The interim forwarding gate of W-0021 slice 2B (`forwarding_gated`, `gate_reason`) is retired (ADR 0082): an archive forwards exactly what its enabled bindings say, so a new archive simply has no bindings and queues nothing, with no gate to state. The list is keyed by the active archive's **binding names** in listing order — every binding, enabled or not, resolved or not (an unresolved binding still has rows to show and clear); on the adopted archive these are the seeded legacy names. Clear and retry accept only these names.
...
- **Gating:** Always-on.
- **Request:** Path `{name}` (a binding name of the active archive, as the queues readout lists it). No body.
...
- **Errors:** 400 `invalid_forwarder` (empty name); 404 `unknown_forwarder` (not a binding of the active archive); 500 `clear_failed`.
...
- **Gating:** Always-on, but the name must have a **running worker**: the workers node builds one worker per enabled binding of the active archive at daemon start (ADR 0082), so a name with no enabled binding then has no worker and re-arming would only show a `waiting` count that never moves. `PUT /v1/config` changes saved configuration immediately but does not add or stop workers until restart.
- **Request:** Path `{name}` (a binding name of the active archive, matched exactly like clear). No body.
...
- **Errors:** 400 `invalid_forwarder` (empty name); 404 `unknown_forwarder` (not a binding of the active archive); 400 `forwarder_disabled` (no running worker of that name); 500 `retry_failed`.
...
### `GET /v1/logbook`
- **Purpose:** List all logbooks. **Always-on.** Response **200**, JSON array of `types.Logbook`. Errors: 500 `db_error`. Since migration 0012 (ADR 0071) each logbook carries `uuid` — its stable identity for cross-file and cloud use, a UUIDv7 minted by the daemon (on insert, or once at start for rows that predate it) and never accepted from a client; the numeric `id` stays the local key on URLs.
...
- **Response:** **200** `{archives: [{id, label, ownership: "managed"|"legacy"|"external", state: "active"|"pending"|"inactive", last_activation_code?, last_activation_error?, size_bytes?, modified_at?}]}` in catalogue order. `last_activation_code` is the daemon's stable classification of the most recent failed activation (`archive_file_missing`, `archive_file_unreadable`, `archive_no_identity`, `archive_identity_mismatch`, `promotion_persist_failed`, `pending_unclear`, `archive_start_failed`) and `last_activation_error` its plain wording; the SPA shows the wording and never parses it. Paths and error chains stay in the log. `size_bytes` / `modified_at` (RFC 3339 UTC) are the file's stat at listing time — the last write, not a "last opened" — and are absent when the file cannot be stat'ed. A failed candidate is `inactive` with `last_activation_error`; a candidate whose restart could not be requested and whose selector could not be cleared lists as `pending` with the diagnostic — the next restart activates it.
...
- **Errors:** 404 `archive_not_found`; 409 `archive_active` (already the active archive), `activation_in_progress` (an activation is pending in this process), `tx_busy` (message names the FT8 or rig reason), `archive_file_missing` / `archive_file_unreadable` / `archive_no_identity` / `archive_identity_mismatch` (the same classifier the start uses; the message is the plain wording, the path only in the log; an expected refusal is recorded nowhere); 503 `restart_unavailable` (no respawn contract — nothing is sealed or written), `archives_unavailable`; 500 `activation_persist_failed` (definitive write failure: nothing on disk, both seals released), `restart_failed` (the restart could not be requested after the persist: pending cleared, seals released), `pending_unclear` (restart request failed AND the clear failed: seals released, the archive lists as `pending` with the diagnostic, the next restart performs the activation with the ordinary rollback).
...
- **Notes:** Reads via `FetchOperatorEventsWithContext`; `detail` is the stored typed metadata verbatim (never raw provider or rig text — ADR 0076 §2, pinned per kind by the recorder's conversion test). `occurred_at` is the producing boundary's stamp for the alarm and archive-outcome kinds (captured before the recorder's queue); the browser/export and forwarding notification kinds use the write time. Detail shapes per kind:
  ...
  - `notification` / `archive.activated` (`info`) — `{archive_id, label}` (ADR 0071): recorded by the promotion node after the pending → active write succeeds, in the newly active archive's file; never on the 202 alone.
  - `notification` / `archive.activation_failed` (`error`) — `{archive_id, label, code}`: recorded by the last-known-good generation once its events node is up, in the recovered archive's file; `code` is the stable archive failure code (see `GET /v1/qso-archives`), `label` bounded operator text (≤ 80 printable runes), `archive_id` the catalogue UUID. Expected refusals (busy, already active, cancelled) are never events.
...
### `GET /v1/qso-archives/{uuid}/bindings`
- **Purpose:** The ACTIVE archive's destination bindings (ADR 0082, W-0021 5D) as the Forwarding tab renders them: one entry per registered destination type, an aggregate switch state over the archive's live logbooks, and one row per logbook. Credentials never ride this wire; each row and account lists the keys that hold a value.
- **Gating:** Always-on. Bindings are read and edited on the **active** archive only — its file is the one open (ruled 2026-09-25).
- **Request:** Path `{uuid}`. No body.
- **Response:** **200** `{archive_id, archive_label, restart_required: bool, destinations: [{type, display_name, account: {configured: bool, label?, fields_set?: [key…]}, state: "on"|"off"|"mixed", reason?: string, logbooks: [{logbook_id, logbook_uuid, logbook_name, logbook_callsign, bound: bool, enabled: bool, forwarder_name?, credentials_set?: [key…], queue: {waiting, failed, in_flight}}]}]}`. `state` is `on` when every live logbook's binding is enabled, `off` when none is (an absent row counts as off), `mixed` otherwise. `reason` says why the destination cannot be turned on here — no station account (`account.configured` false: an entry of the type with every non-clearable station-scoped field set), or SM Cloud outside the adopted Home archive until the identity-aware server lands (5F). `restart_required` is true when the saved rows differ from the bindings the running daemon started with: edits apply at the next start. `forwarder_name` is the opaque handle the queue endpoints take, never a setting.
- **Errors:** 400 `missing_required_param`; 404 `archive_not_found`; 409 `archive_not_active`; 503 `archives_unavailable` / `bindings_unavailable`.

### `PUT /v1/qso-archives/{uuid}/bindings`
- **Purpose:** Edit bindings of the active archive: only the rows the operator changed, merged onto the stored rows. The **whole candidate is validated first** and every affected row commits in **one transaction**; a refusal names the destination, the logbook and the field and writes nothing. PUTs are **serialized end to end** (read, merge, validate, write under one lock): two concurrent edits of disjoint fields of one binding both survive. Every row that ends **enabled** is synthesized over its station account and **built exactly as the next start will build it**; a binding the daemon could not construct is refused, never discovered as a startup failure.
- **Request:** `{"destinations": [{"type": "qrz", "logbooks": [{"logbook_id": 1, "enabled": true, "credentials"?: {key: value…}, "credentials_clear"?: [key…]}]}]}`. Merge-on-PUT: a blank value keeps the stored one; only the type's logbook-scoped keys are accepted (a station key is refused); `credentials_clear` removes named stored fields, accepts only the type's logbook-scoped keys (a station-scoped or undeclared key is refused, never reported as cleared), refuses a key that is also typed with a value, and is accepted only when the row ends disabled. A row that ends **enabled** must hold every logbook-scoped field that is not clearable, and its destination must be enableable here (see `reason`). A new row is named `<type>.<logbook uuid>`; an existing row keeps its name. Disabling never needs an account.
- **Response:** **200**, the fresh `GET` view (so `restart_required` is current).
- **Errors:** 400 `invalid_field_value` (unknown type, a station key typed or cleared on a binding, an undeclared key, a key both typed and cleared, a logbook listed twice), `binding_field_required`, `binding_not_enableable`, `binding_clear_requires_disabled`, `binding_unusable` (a row ending enabled cannot be constructed — its credentials are incomplete or invalid; the message names the destination and logbook, the constructor's cause is logged, never served); 404 `logbook_not_found`, `archive_not_found`; 409 `archive_not_active`, `logbook_no_identity`, `binding_credentials_corrupt`; 503 as for `GET`.
...
- **Errors:** 400 `invalid_json`/`invalid_field_value`/`forwarder_unusable` (an enabled forwarder whose credentials won't construct — would abort daemon startup; the 400 names the forwarder and fault code only, the constructor's cause stays in the daemon log by contract), plus the config validator's stable `{code, message}` findings (config.md §12 / ADR 0010); **409 `callsign_locked_to_logbook`** (a post-setup station-callsign change that would orphan the default logbook — reconcile logbooks first) / **`default_logbook_callsign_mismatch`** (setup found a logbook already at the default id under a different callsign); 500 `config_write_error`/`db_error`.
...
### `POST /v1/ft8/claim`
- **Purpose:** Select the FT-family profile — `ft8` (15 s slots) or `ft4` (7.5 s slots, ADR 0080) — for the capture sessions that follow. The FT view calls it BEFORE opening `/v1/ft8/events`, because a native `EventSource` cannot surface a refusal code. The same profile is always accepted, whatever else is live. A different profile is refused, and nothing changes, while a subscriber holds a capture, a transmission is in flight, a sequenced session is active, or TX is armed — in that precedence. With an idle subsystem the claim also bypasses the five-second capture linger: the old capture is released (not reused by the next subscriber), the replay cache is dropped, and the next subscriber acquires fresh on the claimed profile.
- **Gating:** **Only when FT8 is enabled.**
- **Request:** `{"mode": "ft8" | "ft4"}` (any case).
- **Response:** **200** `{"mode": "FT8" | "FT4"}` — the active profile after the claim.
- **Errors:** 400 `missing_required_field` / `ft8_profile_unknown`; 409 `ft8_profile_busy` | `ft8_tx_in_flight` | `ft8_session_active` | `ft8_tx_armed`. A 409 body carries `retry_after_ms` when the refusal is the linger of a session the operator just left (the unattended disarm at its expiry ends that session, after which the claim succeeds); the view re-claims when it elapses rather than polling.
...
- **Query:** `mode` (optional, `ft8` | `ft4`, any case) names the profile the client expects. A mismatch with the active profile is refused **409** `ft8_profile_mismatch` BEFORE the connection counts as a subscriber — the belt and braces behind `POST /v1/ft8/claim`, never the primary explanation (an `EventSource` sees only an error).
- **Response:** **200** SSE stream. Events:
  ...
  - `ft8-decode` → `DecodeReport` `{slot: SlotRef, decodes: [{text, freq_hz, dt_s, snr}], dial_mhz?, mode}`. `mode` (`FT8` | `FT4`, ADR 0080) is the profile the slot was decoded on; the daemon's PSK Reporter sink spots under it. `dial_mhz` (added 2026-08-07, review P1) is the rig dial the slot was CAPTURED on, 0/absent when unknown — clients must attribute the decodes to a band from THIS, never from live rig state, for the same reason as `ft8-occupancy`'s: publication lags capture by the decode (~0.7–1.6 s), so a QSY in that gap otherwise files a whole slot's stations on the wrong band (wrong PSK Reporter spots, wrong-band Band Activity rows — the in-window case is already handled by the moved-slot suppression above; this covers the move that POSTDATES the window). The daemon's own PSK Reporter sink spots from this stamp and skips unattributable slots; the SPA withholds rows whose capture band differs from the band the view is on.
  - `ft8-tx` → `TxState` `{armed, transmitting, message?, offset_hz?, error?, disarm_cause?, mode}`. `mode` (`FT8` | `FT4`, ADR 0080) names the active profile on every frame, replay included, so a client derives its slot clock, parity and dial table from it and never from timing. `disarm_cause` is the stable code for the disarm the frame reports (`operator` | `unattended` | `cat_lost` | `shutdown` | `band_change` | `dial_moved`), `""`/absent while armed — cleared when a new arm commits, so a replayed frame can never pair a stale cause with a live arm. It exists because an ARM-ONLY safety teardown (dial nudge with no session active) has no terminal `ft8-qso` frame to carry a reason, so it was invisible outside `smd.log` (dogfood 2026-08-07). All causes ride the wire, `operator` included: which deserve a notice is the client's decision (the SPA is silent for `operator`, for a frame replay with no observed armed→disarmed edge, and for the same-teardown duplicate of a session-end notice — one knob turn, one notice, in BOTH frame orders: live publication is qso-then-tx, but the reconnect replay is tx-then-qso, `lastTx` before `lastQso` in `hub.subscribe`).
  - `ft8-qso` → `QsoStatus` `{active, role?, fd?, type4?, their_call?, state?, next_message?, cq_message?, repeats?, max_repeats?, our_report?, their_report?, dial_freq_mhz?, answer_mode?, answerers?}` `cq_message` (caller frames only, W-0011) is the run's CQ text exactly as transmitted — carried while calling AND while an answerer is worked, so a client that did not start the run (a second tab, a reload mid-run) renders the ladder's CQ rung from the daemon's text and never from its own CQ-token preference. `answer_mode` (caller frames only) names the run's answerer-selection mode so a client can tell an `operator_pick` run from an auto one before any answerer arrives; `answerers` (operator_pick caller frames, ADR 0065) is the candidate list `[{call, snr}]` — stations currently answering the CQ that the run can actually work (encodable, heard within the 3-min staleness bound), oldest first, in BOTH phases (it keeps accumulating while a popped contact is worked); pop one via `POST /v1/ft8/cq/pick`. Since ADR 0067 the list rides EVERY pick session/listing run (not just CQ runs), and pick frames also carry `queue` [{call, snr}] — the BAGGED stations, in bag order, auto-worked by the drain — and `drain_paused` (Stop paused the drain; Resume continues). `held` `{call, grid?, snr, band, mode, submode?, reason}` (operator ruling 2026-09-13, W-0019) names a station the run is HOLDING instead of answering — `reason: worked_before`: already worked on that band and ADIF mode/submode in the session's logbook (the same question as `GET /v1/contest-dupe`, asked daemon-side through an injected read). The run carries on around it; the operator answers anyway with `POST /v1/ft8/cq/pick` naming that call (accepted in auto modes too while the hold stands; the contact then reaches the sink as an explicit repeat) or declines it with `POST /v1/ft8/qso/next` (excluded for the rest of the run; the SPA labels this "Ignore"). A hold also ends when a contact commits, when the station is unheard past the 3-min staleness bound, or when the run ends. A failed or unknown lookup never holds. `dial_freq_mhz` is the rig dial PINNED to the session at start — the frequency the contact will be logged on. Clients must attribute a contact to a band from THIS, not from live rig state: the rig and FT8 status are independent streams, so a band change mid-contact (or a skew between them) otherwise files the contact under the wrong band. (`fd:true` marks an ARRL Field Day session; `type4:true` marks a reduced type-4 nonstandard/compound-call session — bare-calls→RR73→73, no grid/report rungs, ADR 0048). `end_reason` appears ONLY on a terminal `active:false` frame and only when the operator did not cause the end. Stable codes, two families: the daemon could no longer confirm the rig's frequency (`dial_moved` | `dial_unknown`), or it could not KEY (`tx_not_armed` — TX was no longer armed; `tx_bad_message` — the next message will never encode, e.g. a compound call SM cannot answer). `band_change` is the one exception to "the operator did not cause it": they moved the rig, and saying it drifted would be a small lie. Absent for an abandon, a TX disarm, or a completed contact — all three are the operator's own doing, and those are named in `smd.log` instead (`reason: operator` | `tx_disarmed`). Clients must render unknown codes through a CAUSE-AGNOSTIC fallback: a frequency-specific default became false the moment the `tx_*` codes existed (2026-08-04). Clients should say it out loud: without it a deliberate safety stop is indistinguishable from a hang (dogfood 2026-07-27 — the first on-air read of a working guard was "moving the dial does not stop TX"). `max_repeats` is present exactly when the CURRENT rung is bounded, so a countdown shows iff `max_repeats > 0`: it is absent while plain calling CQ (uncapped by design) and on a "send once" closing rung whose partner has already rogered (`internal/ft8/finalrung.go`).
  - `ft8-logged` → `LoggedQso` `{uuid, callsign, freq_hz, band, rst_sent, rst_rcvd, mode, submode?, time_on:"HH:MM", qso_date:"YYYY-MM-DD", gridsquare, country}`. `mode`/`submode` are the ADIF pair the contact was filed under: `FT8`, or `MFSK`/`FT4` for an FT4 exchange (ADIF 3.1.5 lists FT4 as a submode of MFSK).
...
- **Gating:** **Only when FT8 is enabled.** Requires TX armed.
- **Request:** Body `{"offset_hz": float64, "operating_freq_mhz": float64, "tx_parity"?: "even"|"odd", "answer_mode"?: "auto_first"|"auto_strongest"|"operator_pick", "cq_modifier"?: string}`. Callsign/grid resolved server-side. `cq_modifier` (W-0011, 2026-09-12) is the optional CQ token that rides between CQ and our call — `"AF"` → `CQ AF 7Q5MLV KH78` — one to four letters or exactly three digits, the shapes the protocol packs into the first 28-bit field; trimmed and upper-cased; anything else → `invalid_field_value` 400 naming the field, before the arm check. The sequencer re-validates the composed CQ through the encoder round trip it already runs, so a token that passes the wire rule but fails to pack still ends as `ft8_tx_bad_message` 400 with no session started. Operating state per Call-CQ session, like `tx_parity`; the app keeps it behind a per-session enable. `tx_parity` is the operator's chosen CQ slot parity (WSJT-X "Tx even/1st" — `even` = :00/:30, `odd` = :15/:45); **omitted/empty/any-other value = call CQ on the next slot regardless of parity** (the default, fastest first CQ). `answer_mode` is the SESSION's answerer-selection mode (ADR 0066, the TX control bar's Answer selector) — omitted/empty = the config default; junk → `invalid_field_value` 400. Both are operating state, sent per session — not persisted daemon settings. Caller-side only (answering a CQ forces the opposite parity).
- **Response:** **202 Accepted** (progress via `ft8-qso` SSE).
...
- **Errors:** (while an `ft8-qso` frame carries `held`, Next declines that station for the run and returns 202 in any run shape.) **409 `ft8_no_answerer`** when no answerer is being worked — idle, a Call-CQ run that is merely calling, or an answer/work session (whose Next is `qso/skip`). A third distinct refusal alongside `ft8_no_active_qso` and `ft8_rung_not_skippable`.
