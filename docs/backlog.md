...
**P1 — correctness (verified open; code-authoritative audit reconciliation 2026-08-20)**

_Surfaced from the internal audits + the frontend-app review, each verified against current code. The earlier "bounded P1 reconciliation closed 2026-08-18" covered only the session-192/193 behavioral retest — NOT these correctness findings. Ranked by operator urgency; **W-IDs are assigned only when the dossier is created**, not reserved here; full detail stays in the cold audits (`docs/reviews/`)._

1. **CC-1 — startup erases unknown config keys, automatically and silently.** Every boot rewrites `config.json` via a typed round-trip that drops unrecognized JSON members (`cmd/smd/main.go:233` → `internal/config/config.go:2092`), then `UnknownKeys` warns *after* reading the already-scrubbed file (`cmd/smd/lifecycle_adapters.go:192`). The former EH-3 blocker is fixed, so this is independently actionable. Operator decision: preserve-and-warn vs. reject unknown keys before any write. [configuration-contract CC-1]
2. **PT-1 — equal-version SM Cloud write corrupts the backup/reconciliation contract.** An equal `(uuid,modified_at,revision)` tuple unconditionally overwrites the stored payload (`internal/cloud/store/store.go:204`), and the reconcile hash covers only that tuple (`internal/cloud/reconcile/reconcile.go:53`), so divergent content is permanently undetectable by routine reconciliation. [persistence-transaction PT-1]
3. **F-01 — malformed/partial station-config baseline erases sibling fields.** Malformed or partial station-config success is accepted as an authoritative whole-block baseline, allowing omitted station fields to be erased on Save. (A fully-empty callsign is rejected `409 callsign_locked_to_logbook`, `internal/api/handler_config.go:671`; the reachable destructive case retains the bound callsign but omits siblings.) `frontend/app/src/lib/api/config.ts:68`, `config/station.svelte.ts:97`. [frontend-app-review F-01]
4. **PT-2 — concurrent QSO delete loses the latest state and records a false preimage.** Delete has no revision CAS and marshals the handler's stale snapshot as the append-only `qso_history` preimage (`internal/qsoservice/delete.go:54,76`; `DeleteQsoByIDTx` has no revision predicate, `internal/database/sqlite/api_context.go:2909`). [persistence-transaction PT-2]
5. **F-02 — operator-triggered re-enrich races the callsign it forwards.** The QSO-edit re-enrich has no abort/generation guard and the callsign field stays editable during the lookup, so callsign A's enrichment (incl. its zones/DXCC) can be persisted and re-forwarded onto B. `frontend/app/src/lib/logbook/EditQsoModal.svelte:83,134`. [frontend-app-review F-02]

_In-flight/validate arc: none open — the session-192/193 behavioral-retest batch was closed by operator decision 2026-08-18._

**P2 — next features (open one workstream per active focus)**
- **W-0001 · P2 release gate · OPEN — [Durable operator notification history](work/W-0001-durable-notifications.md).** Keep important notification facts retrievable after the SPA's bounded toast expires or its tab closes, without mirroring `smd.log`.
- **W-0002 · P2 validation gate · OPEN — [Validate the FT8 reduced type-4 ladder on air](work/W-0002-ft8-type4-on-air-validation.md).** Confirm one operator-initiated end-to-end contact with a real nonstandard station before ADR 0048 moves from Proposed to Accepted.
- **W-0003 · P2 post-ship · OPEN — [Complete the app shell and retire legacy operator SPAs](work/W-0003-retire-legacy-operator-spas.md).** `frontend/app` is now the sole embedded operator SPA; finish canonical-root/deep-link ownership and route-level lazy loading while keeping the zero-JS manual separate.
- **W-0004 · P2 post-ship · OPEN — [Complete app UI cohesion and ambient build identity](work/W-0004-complete-app-ui-cohesion.md).** Replace the stale hard-coded build label, preserve the shipped light/dark system, and settle the FT8 occupancy and optional named-palette decisions.
- **W-0005 · P2 · OPEN — [Operator-triggered forwarder queue clearing](work/W-0005-operator-triggered-forwarder-queue-clearing.md).** The history-preserving discard primitive (`DiscardQueuedUploadsForForwarderWithContext`) and the disabled-forwarder startup purge already exist; what remains is the operator-triggered API + Settings → Forwarding UI and a coordination policy for an enabled forwarder's in-flight batch.

_**P2 — correctness (verified open; code-authoritative audit reconciliation 2026-08-20).** Deduplicated across the internal audits + frontend review; cross-review ownership preserved (PT-1 owns equal-version cloud conflicts, AW-6 only caps batch work; PT-2 owns stale-delete history, AW-3 owns false-update history; PT-3 owns session-email stamps). Detail in the cold audits; not yet dossiered; dossiers cite `docs/v2-design/api-endpoints.md`, never the historical `api.md`._
- _Persistence:_ **PT-3** session-email stamps a revision that was not sent and over-reports `emailed` (`api_context.go:2289`, `handler_session_email.go:264`) — owns the stamp correctness that GATES any session-email templating · **PT-4** concurrent partial logbook PATCH lost-update (`handler_logbook.go:105`, `api_context.go:1851`) · **PT-5** import fallback proceeds after an unverified `rollbackTx` (`submit_batch.go:198`, `rollback.go:16`) — **re-scoped off EH-6 (now fixed 2026-08-15)** · **durable config replacement (PT-6 + CC-5)** the atomic-rename write is not crash-durable (no temp/dir fsync) and reuses a fixed `.tmp` name that can block later writes (`config.go:700-713`) — operator decision: how to report a post-rename directory-sync failure.
- _Config contract:_ **CC-2** unknown-key detection stops at struct slices, so typos inside `rigs[]`/`forwarders[]`/`lookup.chain[]` go undetected (`config.go:1454`) — amplifies CC-1 · **CC-3** `Config.Validate` omits `datastore`/`logging` (`validate.go:62`) · **CC-4** the persistence primitives (`Update`/`UpdateInMemoryThenPersist`) don't enforce normalize→validate (`config.go:2079`). CC-3/CC-4 bring code into agreement with the intended contract at `docs/v2-design/config.md:251` (which is NOT current behavior).
- _API wire contract (route in this order):_ **AW-2** presence-aware command booleans — Tune/FT8-arm/FT8-skip value-booleans execute `false` on a missing/misspelled field, 202 falsely acks (fails safe re RF — disarms, never keys) · **AW-3** genuine no-op QSO PATCH — empty/unchanged PATCH still bumps revision, appends history, re-arms forwarding, emits an event (operator decision: 200 no-op vs 400) · **AW-5 + AW-6** one bounded SM Cloud ingest-hardening item — oversized JSON → 400 leaking decoder text (`server.go:230`), and QSO batches have no row cap while evidence caps at 1000 (operator decision: max batch rows) · **AW-4** JSON `/v1/` 404/405 routing fallbacks (both muxes rely on `http.ServeMux` plain text) · **AW-1** the larger UUID compatibility migration — wire/SSE still carry integer ids (operator decision: compatibility/removal release).
- _Frontend:_ **F-03** inconsistent runtime wire validation at success/safety boundaries — non-`duplicate` submit → `stored`, SSE `JSON.parse as T` with only null-checks (`qso.ts:102`, `ft8-sse.ts:171`) · **F-04** writes flatten an ambiguous transport timeout into a definite failure — a shared outcome-unknown/reconcile type is missing on `config.ts`/`rig-command.ts`/`ft8qso.ts`/`uploads.ts` (only ft8-config + the rigs/CAT reconcile paths honor `timedOut`).

_**Maintainability / architecture (audit reconciliation 2026-08-20; P2/P3, not correctness).**_
- **PB-1..4** incomplete boundary guards (`types` stdlib-only / FT8-direction / cloud-independence unguarded), lookup orchestrator + smcloud own more than a leaf should, dead exported `utils.DXCCFromISO2` · **MT-1..7** `logging_debug` build nil-panic (opt-in, not shipped/CI-gated), complexity/dupl gates can't ratchet exempted hotspots, 26 context-free SQLite wrappers + test-only machinery compiled into prod, non-reproducible model generation, absent httpkit/hardware leaf tests · **F-05 + F-07 (first browser-proof portion)** modals don't own keyboard/focus/Escape (a11y; RF keys stood down, no RF leak) paired with the first real browser/a11y test layer (jsdom-only today) · **F-08** session hydration validates only the outer array (P3).
- _Needs-trigger:_ **LC-5** the SQLite `Open/Close` lock-free `!isOpen` fast-path race — latent, never exercised concurrently today; matters only if a future reload/restart path calls them concurrently (`service.go:180`) · **ST-3b** the non-loopback TCP control plane is unauthenticated — **deferred by design, gated behind the config-file-only `allow_insecure_network` ack; tracked in [ADR 0069](decisions/0069-non-loopback-tcp-acknowledgement.md)**, not an active P2.
- _Routed into existing dossiers (not standalone):_ **F-06** default-route → empty Dashboard placeholder → [W-0003](work/W-0003-retire-legacy-operator-spas.md)'s open canonical-root/default-route decision (must not authorize the separately-backlogged whole-log Dashboard) · **F-09** split by ownership — build-identity → [W-0004](work/W-0004-complete-app-ui-cohesion.md); stale scaffold/route docs (`vite.config.ts`, app README) → W-0003 consolidation cleanup.
- _FT8 — **PARKED 2026-07-31 (operator): "none of these 6 are pressing or something I recognise as needing" — do NOT present this cluster as a session target list. Revisit only when the operator names a specific item.** The verified detail below is accurate as of 2026-07-31 and is kept because it cost a full re-verification pass — but it is a snapshot, not a standing sweep, so re-verify before building if one is picked up much later. **RE-VERIFIED AGAINST THE CODE 2026-07-31** (the 2026-07-18 sweep had expired: the retired logging SPA was removed 2026-07-21, three days later, killing three sub-items). Realistic target list is 6 buildable + 1 decision, not 11._ **OPEN, buildable now:** type-4 free-text messages (`modulate.go:142` still calls `goft8.EncodeStandardMessage` — structured only; no entry UX) · type-4 work-a-caller trigger (daemon `StartWorkCallerT4` exists at `type4_sequencer.go:217` but is UNREACHABLE — `handler_ft8_qso.go:113,167` accepts `mode:"type4"` only on the ANSWER path and `startFt8WorkCaller` takes no mode arg; still deferred on hashed-us ambiguity, so DECIDE before building) · callsign ignore list (no implementation, daemon or SPA) · Call-CQ waiting feedback (partial — `Ft8Operate.svelte:90` renders `· sent ×N`; nothing says the run is unanswered) · Call-CQ layer-2 recency pool (layer 1 shipped 2026-07-17; still no candidate pool in `Sequencer`, only the same-slot rescan) · work-path opening: prefer a clean next-slot start over the truncated immediate fire (`work_sequencer.go:74` and `:329` still call `fireOpening`). **BLOCKED:** attempt-limit control — everything exists EXCEPT the UI (`handler_config.go:100` `ft8_max_repeats` + range validation, `:774` pushes it live via `SetMaxRepeats`); gated on the app Settings view, which is still a placeholder. **DECISION, not a build:** offset-picker snap — the design moved underneath it. `Ft8OccupancyStrip.svelte:63` already snaps the daemon ★ pick to its cell and `Ft8OccupancySpectrum.svelte:7` deliberately does NOT snap ("click ANYWHERE"); the open question is whether an arbitrary-click snap is still wanted at all. **DEAD — do not pick these up:** ~~footer info-strip rehome~~ (`rxCaption` existed ONLY in the retired logging SPA) · ~~shift+ctrl freq-step parity~~ (`RigKeys.svelte` is shell-global in `frontend/app` and covers FT8; the gap was logging's `QsoPanel`) · ~~accumulate-mode slot-grouped Rx pane~~ (`rxDecodes` does not exist in `frontend/app` and there is no standalone Rx-pane component — almost certainly logging-era too, but CONFIRM what the app's Rx view renders before deleting this entry rather than assuming). **Also FT8 and also not on-air, filed elsewhere:** auto-work pile-up ADR · FT8 session-reconnect reconcile (see the smcloud line) · FT8 duplicate-QSO detection + merge (line below; overlaps the logging workstream).
- _Forwarding / data (verified in the 2026-07-18 sweep; forwarder queue clearing migrated to W-0005):_ configurable session-email subject/body (open — still hardcoded in `handler_session_email.go`, with in-code comments pointing here) · **ClubLog putlogs.php bulk-backfill path (logged 2026-07-19, from the API-key helpdesk exchange):** ClubLog's grant condition is that realtime.php never carries catch-up batches of pre-existing QSOs (anti-pattern → key blocked; confirmed back to them 2026-07-19) — but SM's Logbook backfill rides the worker → realtime.php one QSO at a time, so pointing it at ClubLog for a historical set would break the promise. Interim rule (documented in the inbox note): ClubLog history = manual ADIF upload on clublog.org. **The refusal is ENFORCED as of 2026-07-19 (refined same day per review — retry-aware):** `forwarding.RegisterNoBulkBackfill` (clublog registers in init) → `qsoservice.EnqueueUploads` distinguishes PER ROW via queue history: a QSO with prior clublog queue rows was a live upload, so re-arming it is legitimate realtime usage (the 403-era Terminal rows' recovery path — review caught that the first blanket block severed it); a history-less row is backfill → refused into `skipped_no_history` (no queue row written). The logbook SPA shows an amber "Retry failed uploads to clublog" button for such destinations (tooltip + notice explain what was skipped; the "Not on clublog" gap-browse stays — it's how the export set is assembled). Deletes (delete.php) and live logging-time enqueues are unaffected. Same review round also fixed the gzip layer: `gzipResponseWriter.Unwrap()` (without it `http.ResponseController` couldn't extend the export write deadline past the server-wide 2 min — every default Go client accepts gzip, so slow restores would truncate mid-JSON) + proper Accept-Encoding negotiation (q-values/case/wildcard; `gzip;q=0` no longer served gzip) + `Vary` on identity responses too. The proper fix: the clublog forwarder gains a putlogs.php batch route and the backfill path routes ClubLog-bound sets through it (batch ADIF POST) instead of per-QSO realtime rows — then 7Q8AC-style operators get in-app backfill without the manual step. Design notes: (a) putlogs semantics differ (whole-log ADIF upload, server-side dedupe), so the backfill result reporting (enqueued/skipped counts) needs rethinking for that route; (b) **durable retry provenance** (review round 2 #2, accepted limitation for now): the retry-only gate keys on qso_upload rows, which are working state — the ADR 0039 startup purge deletes a disabled forwarder's non-uploaded rows, so disabling ClubLog mid-403 loses the failed rows' retry eligibility (degraded path = the ADIF manual upload, which is ClubLog's blessed route anyway — documented at the gate in `enqueue.go`). The putlogs route dissolves this: once bulk is legal in-app, the history distinction stops mattering for recovery.
...
- _Logging-audit residuals (audit CLOSED 2026-08-20; record: [`internal-codebase-logging-gaps.md`](reviews/internal-codebase-logging-gaps.md)):_ **F14** — QRZ/ClubLog accepted-upstream dispositions collapse to generic `forwarding: success` (P3, adjacent-code polish; the `Result.Detail` mechanism now exists from L12/smcloud, so extending it to QRZ/ClubLog is cheap — do it when either forwarder is next open) · **L6 handler breadcrumbs** — add `request_id` to the ~25 direct handler `Warn`/`Error` logs across 8 daemon handler files (P3, adjacent API-work sweep; the 5xx/panic correlation paths already carry it via `ca7b9e60` — do NOT schedule a standalone 25-site edit) · **F13** — boot-time Warn for a keyless-ClubLog build (needs-trigger only; the first-outage warning is already bounded + explanatory via L11 — add when ClubLog construction/config is next touched). FT8-11 (fireOpening deferral SSE frame) is folded into the FT8 work-a-caller opening item below, not a logging item. Q7 (restore probe) needs no routing — already fixed in code.

- **`internal/pskreporter/ipfix.go:150` (`appendVarStr`) — sanitise outbound IPFIX string fields (control bytes / tab / newline). DEFERRED HARDENING (operator-queued 2026-08-13, from the PSK Reporter incident).** The variable-length string fields (sender `Call`/`Grid`/`Mode`; receiver `Call`/`Locator`/`Software`/`Antenna`) all route through `appendVarStr`, which length-prefixes and truncates at 254 bytes but never strips control chars, tabs or newlines — they pass straight through. Legal on the IPFIX wire (fields are self-delimiting, so it cannot desync a conformant collector's framing), but a stray tab/newline can corrupt a downstream *text* consumer such as PSK Reporter's TSV export. **NOT a proven cause** of the 2026-08 collector trouble — the maintainer (Philip Gladstone) diagnosed a server-side memory leak in the odd-IP shard, already fixed by his own protection; queued as plain client hygiene, not a fix for his bug (and the length-prefix point makes a "control byte spun the parser to OOM" story a weak fit). _Change:_ strip (or replace-with-space) C0 controls (< 0x20), DEL (0x7F) and embedded tab/newline at the single encode chokepoint (`appendVarStr`), so every string field is covered once. `Call`/`Grid`/`Mode` are already near-constrained by FT8's alphabet; operator-free-text `Software`/`Antenna` are the real risk. _Acceptance (operator-observable):_ a Spot/Receiver whose fields contain a tab, a newline and a 0x01 byte encodes to a datagram whose corresponding field bytes contain none of those, with the visible content otherwise unchanged; assert on the emitted datagram, not internals. TDD, RED first. Does NOT touch the live TX path. _Trigger:_ next time we touch `internal/pskreporter`, or if any downstream consumer is shown to choke — otherwise low. _OPEN DECISION (operator, settle before building):_ strip vs replace-with-space (strip is simplest; replace preserves token boundaries in a display).
...
- **FT8 semi-auto response to a session watch-list — SET ASIDE 2026-06-12 in favour
  ...
  *daemon-initiated* operation (QEX §9 forbids robotic/unattended; SM's
  ...
  started). See the scope note at the end of this file, AGENTS.md's FT8 rule,
  and `docs/ft8.md`.
...
- **UI consistency across SPAs — shared theme layer.** _(Reframed 2026-07-06 by
  ...
  genuinely good token layer — a Tailwind
...
- **FT8 work-a-caller / Resume opening — prefer a clean next-slot start over a truncated
  ...
  **Logging-audit residual (FT8-11, audit closed 2026-08-20):** the same deferral also
  publishes no SSE frame, so the operator sees no explanation for the skipped rung — the
  operator-facing half of this opening rework, NOT a logging-gate item (the daemon-side
  deferral logging shipped 2026-08-07). Must NOT reactivate the parked FT8 cluster.
