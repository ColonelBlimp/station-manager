...
- _smcloud hardening — **pre-Phase-2 gate** (operator-flagged 2026-07-18):_ **`cmd/smcloud` needs rate limiting before anything internet-facing.** The 2026-07-18 hardening batch bounded the DB pool (5 conns), but nothing bounds REQUESTS: `/v1/health` is unauthenticated and pings Postgres per hit, and excessive traffic piles up unbounded HTTP handler goroutines WAITING for pool connections — the pool protects Postgres, not the process. **Design (operator, 2026-07-18), two layers with distinct jobs:** (1) **per-IP rate limiting at the reverse proxy** (Caddy) — the proxy sees real client IPs and is the right place for abuse-shaped policy; an in-binary per-IP tracker would see the proxy's address anyway. (2) **a global in-process concurrency limit** in smcloud (bounded semaphore around the handler chain, ~2–4× the pool size; over-limit → 503 + Retry-After) — caps goroutine/memory growth and keeps the binary safe even when someone runs it without the proxy. Sits with the ADR 0040 security assessment that already gates Phase 2 (VPS/Caddy); the Caddyfile example gains the rate-limit block in the same change. · **Streaming/paginated export (review 3 #4):** `/v1/export` buffers every payload then JSON-encodes the lot; bounded today (single tenant, ~5.6k QSOs ≈ a few MB) but a VPS-scale memory liability — stream rows inside the `ExportSnapshot` transaction when the Phase-2 batch lands. **FT8 session reconnect reconcile** — `ft8-logged` has no backlog, so a QSO completed during an SSE gap is durable in the logbook but missing from the session list / export / worked markers. **Design note (the trap):** do NOT naively replay-cache the event daemon-side — a fresh page load hours later would receive the stale event into an empty session list, and the uuid dedup can't catch that (the new session doesn't hold the row). Correct shapes: SPA reconnect reconcile (on re-open, fetch QSOs since session start, merge by uuid) or a time-bounded replay the SPA can age-filter. · **Write idempotency for QSO submit + session email (daemon)** — a timed-out write is ambiguous (may have committed / SMTP may have accepted); the SPA now *says* "outcome unknown" (shipped), but resolving the ambiguity needs a daemon-side idempotency token (client-generated key the daemon dedups on, with a retrievable outcome) so a retry is safe instead of merely warned about. Email is the sharper case (a real duplicate lands in a real inbox).
