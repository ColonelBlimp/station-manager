...
- **Bridge TX-safety revision (2026-07-18 review, findings 1/2/4 — Critical/High; the stuck-TX incident's root fix).** Design settled in **ADR 0051 (Proposed)**: tristate TX state (idle/keyed/**uncertain** — only positive confirmation clears; CI-V ACK, or a new rigdef `read_tx_status` `TX;` read-back for the Yaesu family) · persistent **`tx-alarm`** SSE + SPA "CHECK YOUR RADIO" banner on unconfirmed/liveness-lost TX · teardown takes `keyMu` (closes the teardown-vs-TX1 race) · **unconditional defensive tx_off on every identity-confirmed connection**, deleting `strandedKeyed` (supersedes the ADR 0042 accepted residual — survives daemon restarts, which the flag never could). Flip 0051 → Accepted at build. Companion facts: rig TOT documented in the manual (done); coverage gap `ft8TxAutoOff` 0% / `tuneAutoOff` 28.6% gets closed with the rework.
- **Bridge TX-safety companion batch (review findings 3/5/6/7 — High/Medium/Low, contained):** one strike-aware live/write-ready predicate enforced in `TxReady`/`KeyFt8Tx`/`StartTune`/`SendCommands` (a 2-strike "dead" rig is currently still keyable) · skip/defer CI-V snapshots+polls while keyed (a snapshot holds `cmdMu` ~8 s ahead of TX0; Yaesu unaffected) · per-transition generation token so a stale auto-off retry can't unkey a NEWER transmission (`tune.go:305`/`ft8tx.go:246` re-arm on the boolean only) · classify identity BEFORE latching `identityVerified` (`pipeline.go:699` — a garbled first ID permanently write-blocks the instance). Subsumes the two old Bridge/TX-hardening P2 items (generation counter · identity poisoning).
