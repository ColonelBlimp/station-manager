...
1. **CC-1 — reject unknown config keys before any write** → [ADR 0074](decisions/0074-reject-unknown-config-keys-before-any-write.md) · dossier [W-0006](work/W-0006-reject-unknown-config-keys.md). Ruling ratified 2026-08-20 (reverses the 2026-06-19 warning-only ruling; premise removed by the schema-version downgrade guard), absorbing CC-2 (struct-slice detection). The dossier owns scope + acceptance criteria; `config.md` + code + TDD land in the implementation commit. [configuration-contract CC-1/CC-2]
