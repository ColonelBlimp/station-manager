...
- _None open — the session-192/193 behavioral-retest batch was closed by operator decision on 2026-08-18, completing the bounded P1 reconciliation._

**P2 — next features (open one workstream per active focus)**
- **RECONCILIATION 2026-08-11 — the five SHIP-GATE logging-audit entries below are BADLY STALE; read this before picking any of them.** A full per-package cross-check of every finding against current code ran 2026-08-11: **32 of 67 findings were already shipped** at that point (with pinning tests), including nearly every "top item" the entries still call open (api A7, qsoservice Q1/Q7, ft8 1-8/11-14, forwarding F1/F4/F6/F9, bridge B1/B2/B3). **Current open sets (each `reviews/*-logging-gaps.md` file now carries a per-file status header — trust it over the mega-bullets below):** _bridge_ — **10/14 DONE** (B1-B4, B6-B11; commits `568742ed`/`df1db6ab`/`1408edb1`, 2026-08-11), OPEN **B5** (CIV mid-batch), **B13** (tune-restore encode drops), **B14** (freq/power parse) — cheap, need test scaffolding — plus **B12** (needs-decision). _api_ — **9/12 DONE** (was 7; **A11 both halves + A6 FIXED 2026-08-12, working tree**: `mergeForwarders` preserves an undecodable stored-creds blob instead of dropping it; `credentialKeysSet`/`noteForwarderCredCorruption` warn once per corrupt forwarder at GET; `recoverPanic` flags a committed/truncated response and skips the doomed envelope — all TDD + reversion-proved), OPEN **A5/A10/A12 (needs-decision)**. _qsoservice_ — **Q4/Q5/Q6 (committed `2ddb2809`) + Q8/Q9/Q10 (working tree) FIXED 2026-08-12**: stamp-sync→Info; `forwarded_to` on QSO stored/soft-deleted; 14 rollback sites Warn via `rollbackTx`; import-batch fallback logs its triggering error; `forced`+`source` on QSO stored; `SubmitImportBatch` logs a completion summary — all TDD + reversion-proved. **OPEN: Q7 only** — `Restore`'s existence probe treats a DB failure as "not found" (`restore.go:60`), same defect class as api A7; **NEEDS-DECISION** (log the non-not-found error, and should Restore also *refuse* rather than write? — operator's call, don't decide it). _ft8_ — **14/14 DONE** (#9 idle-inhibit held-interval + #10 dial-refusal vs audio-failure message FIXED 2026-08-12, working tree — both TDD + reversion-proved; #10 is log-classification only, no keying logic touched). ft8 logging-gaps CLEARED. _forwarding_ — **F5/F7/F8/F15/F16 FIXED 2026-08-12 (working tree)**: internal-transient retry/exhaust logging (F5); soft-delete/missing-QSO terminal Warn (F7); reconcile partial-mutation record (F8, via a `runOnceOverride` test seam); worker-start retry policy (F15); unrecognised-outcome error (F16) — all TDD + reversion-proved. OPEN **F3-age** (cheap) + **F2/F10/F11/F12/F13/F14/F17 (needs-decision)**. One cross-cutting DECISION gates several: "what does a keyed transmission record" (api A10 / bridge B12 / ft8 #6 already shipped its half) · A5/A12 log-levels · a `forwarding.Result` disposition field (F11/F14). **Do NOT re-audit from scratch — build from these open sets.**
- **WHOLE-`internal/` logging consolidation — NEW 2026-08-12 ([`reviews/internal-codebase-logging-gaps.md`](reviews/internal-codebase-logging-gaps.md), L1–L13 + H1–H4).** An operator-directed source-level audit of all 287 production Go files under `internal/` (~68k lines), re-cross-checked against the working tree — it does NOT copy the per-package open labels (several had already shipped). It re-numbers findings by **release gate**, not by package: **P0 blockers L1–L3** (**L1 FIXED 2026-08-12, working tree** — health-tracking fan-out replaces `io.MultiWriter`: delivers to all targets despite one failing, non-recursive journald fallback on the degraded/recovered edges + 5-min write-driven heartbeat, `/v1/healthz` reports `logging:degraded` at 200; TDD, 7 tests, reversion-proved. OPEN **L2/L3**) (the logging system can't report failure of its own file sink; `io.MultiWriter` also stops at the first failed writer · evidence retention/capacity swallows SQL/FS errors as valid-looking **zeros** · audio + evidence queue-loss silent or misclassified — `writer_error` reused for a queue-full drop where no write was attempted). **P1 L4–L9** (rig-mutation audit trail: non-atomic CI-V batch + generic-command outcome + normal tune start/stop leave no durable record · malformed CAT leaves safety-relevant dial/power state stale silently · HTTP request correlation · post-commit forwarding-hook `OnQsoStamped` panic paints a false "reset to retry" narrative over an already-committed upload · local evidence quarantine invisible · service-lifetime goroutines bypass `safego` panic capture). **P2 L10–L13 · P3 H1–H4.** The file carries a recommended gate order and a **"test standard for closing a finding"** (assert the two nearest confusable states differ in the diagnosis-driving field; assert bounded volume on every retry/high-frequency path) — that standard is the bar for any fix off this list. **THIS-SESSION OVERLAP, verify before building: L8 (quarantine invisible) is SHIPPED as C1b** (per-kind count breakdown is the one refinement left); **L6's cloud half is SHIPPED as C4/C5** (app access log + request-id + tenant on list/export/evidence) — **L6 remainder OPEN:** cloud structured panic-recovery + the DAEMON-side request-id at `api/middleware.go`. Several L-findings also appear to overlap existing per-package opens (L4↔api A10 / bridge B12+B5 · L5↔bridge B14 · L11/L12↔forwarding F11/F14 · H1↔bridge B13) — treat this as the authoritative consolidated view, the per-package files as the file:line detail, and **verify each finding against the committed tree before picking it**. Fold in + delete once the gate is cleared.
- **CLOUD logging audit — the SIXTH package area, NEW 2026-08-11 ([`reviews/cloud-logging-gaps.md`](reviews/cloud-logging-gaps.md), C1–C6).** `cmd/smcloud` + `internal/cloud` were never part of the 2026-08-01 five-package sweep; a codex/cloud logging review surfaced six gaps (transcribed from `.codex-reviews/20260811-001.txt`, file:line spot-checked). **The two High false-success bugs are FIXED 2026-08-12 (working tree):** **C1 (server half)** — `handlePutEvidence` now logs the per-batch outcome breakdown (accepted/rejected/tombstoned/… counts); **C2** — `gzipMiddleware` captures the discarded `gz.Close()` error and Warns on a failed flush. Both TDD (real Postgres for C1) + reversion-proved. **C3/C5/C6 also FIXED 2026-08-12 (working tree)**: C3 startup/migration audit trail (version on base logger, migration duration, listening/stopped markers, structured boot-failure); C5 tenant_id on the logbook-list + pre-stream-export error logs (evidence.go done by C1); C6 `http.Server.ErrorLog`→slog — build/suite-verified (error/startup paths, no dedicated tests). **C1b also FIXED 2026-08-12 (working tree)**: `applyOutcomes` logs a bounded per-batch quarantine Warn. **C4 also FIXED 2026-08-12 (working tree)**: new OUTERMOST `access.go` middleware — bounded untrusted `request_id` (16–64 `[A-Za-z0-9_-]`, else crypto-random; never an auth input), one access line per request (method/path/status/duration/bytes/request_id/tenant_id/remote), `request_id` in context + response `X-Request-Id` header + the handler error lines, health/version at Debug; Caddy overwrites client `X-Request-Id` at the boundary (`docs/smcloud-deploy.md`). TDD (real Postgres) + reversion-proved (4 acceptance tests). **The whole cloud audit (C1–C6 + C1b) is now cleared in the working tree** — awaiting commit. F11 (forwarding) is the client-side cross-ref, not a new item. Fold in + delete the file once committed.
