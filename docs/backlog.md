...
- ~~**Bridge TX-safety round 3 (2026-07-19 review, 5 findings — all verified real)**~~ **BUILT 2026-07-19:** (P1) **restore gated on positive RX confirmation** — `waitTxConfirm` (a per-cycle done-channel the confirm machinery closes on idle OR alarm) now gates the tune power/mode restore AND the FT8 mode restore; unconfirmed/alarmed → restore SKIPPED (clamped-power RTTY + standing banner is the safe state; previously a fixed 150 ms settle wrote `PC` full power even if the rig had missed `TX0;` — amp-damage territory). CI-V unaffected (ACK confirms synchronously). · (P1) **cause-aware teardown** — `unkeyOnTeardown` takes a fault flag (`ctx.Err()==nil` at teardown = error-shaped exit): a FAULTED pipeline's write-accepted unkey now ALARMS (the pipe just proved untrustworthy and the supervisor may never reconnect — the incident shape); the quiet-uncertain ADR 0051 trade is retained for healthy ctx-cancelled shutdowns only. · (P2) **per-VFO dial knownness** — `dialKnown` deleted; `CurrentDialMHz` returns ok=false when the SELECTED VFO is undecoded instead of falling back to VFO-A (wrong-band FT8 logs). · (P3) **tune power def-floor validation** — New + `ResolveTune` dry-run the configured power against the rigdef (`validateTunePowerForDef`; FTdx10/FT-710 PC floor = 5 W) and fall back to the 20 W default with a warning (power_w:1 previously advertised tune:true while every StartTune failed at encode). · (P3) **multi-tab count race** — `clientsMu` holds join/leave + count + publish as one ordered unit (a stale count could previously be the LAST event, sticking a lone tab's banner). Tests: unconfirmed-skip (tune + FT8 + TX1-answer variants), fault-teardown alarm, per-VFO unknown, def-floor ResolveTune cases, + an `answerTxStatusQueries` fixture helper so healthy-rig tests confirm (suite back to ~4 s). Full `-race` green. → archive at next sweep.
- ~~**Bridge TX-safety revision (2026-07-18 review, findings 1/2/4)**~~ **BUILT 2026-07-18 (ADR 0051 → Accepted).** Shipped: `txUncertain` confirm-or-alarm model (`txconfirm.go`) — non-ACK unkeys enter uncertainty, cleared only by the rigdef `read_tx_status` answer (FTdx10+FT-710: `TX;`→`TXn;`, both defs updated surgically), CI-V confirms via its ACK, no-query defs fall back to any-rig-data · `tx-alarm` SSE (hub-replayed) + persistent red "CHECK YOUR RADIO" banners in BOTH SPAs (dismiss hides locally, only positive RX confirmation clears; logging SPA via i18n codes) · teardown now holds `keyMu` — the final TX0 is the last serialized wire action, closing the TX1 race · `strandedKeyed` machinery DELETED for the unconditional defensive tx_off on every identity-confirmed connection (stateless, survives daemon restarts; supersedes the ADR 0042 residual, noted there) · key paths + TxReady refuse while unconfirmed (`ErrTxUncertain` → 409 `rig_tx_unconfirmed`) · liveness strike-out while keyed/uncertain alarms immediately. End-to-end recovery pinned through the REAL pipeline (standing alarm → identity confirm → defensive unkey → query → `TX0` answer → clear). **Second review round (8bd88c1b) closed same day:** defensive recovery is UNCONDITIONAL on every confirmed connection with the full confirm-or-alarm cycle (the fresh-restart shape — no in-memory seed needed; write failure alarms + blocks TX), uncertainty set BEFORE identity unlocks (no key can race the recovery window; the wire phase runs async so the CI-V ACK can be delivered by the readLoop), teardown retains `txUncertain` even on an accepted write (write-accepted ≠ confirmed) and also fires when only-uncertain, `SendCommands` refuses while unconfirmed, and the any-rig-data fallback is frame-watermarked (a pre-unkey frame proves nothing). Restart-modeled + silent-rig-alarm + watermark + command-gate tests added; bridge suite 8×-stable under `-race`. Full `-race` green; both SPA suites green. → archive at next sweep. _Original design:_ tristate TX state (idle/keyed/**uncertain** — only positive confirmation clears; CI-V ACK, or a new rigdef `read_tx_status` `TX;` read-back for the Yaesu family) · persistent **`tx-alarm`** SSE + SPA "CHECK YOUR RADIO" banner on unconfirmed/liveness-lost TX · teardown takes `keyMu` (closes the teardown-vs-TX1 race) · **unconditional defensive tx_off on every identity-confirmed connection**, deleting `strandedKeyed` (supersedes the ADR 0042 accepted residual — survives daemon restarts, which the flag never could). Flip 0051 → Accepted at build. Companion facts: rig TOT documented in the manual (done); coverage gap `ft8TxAutoOff` 0% / `tuneAutoOff` 28.6% gets closed with the rework.
...
- ~~**Bridge: the tx-alarm cannot self-clear**~~ **BUILT 2026-07-23** (`internal/bridge/txrecheck.go`): a bounded alarm-driven re-probe loop (first probe 250 ms after the raise, then every 5 s for ~5 min, generation-gated so a clear or a re-raise retires it, cancelled by `Stop` via the retained run context) + `POST /v1/rig/tx/recheck` → `RecheckTx` for the manual path, surfaced as a **Re-check** button beside the banner's existing local Dismiss. Everything writes the `read_tx_status` READ and nothing else — no ack/clear endpoint exists, and only `confirmTxIdle` on the rig's own "in RX" answer may retire the alarm. Tests pin the negative: re-checking leaves `txAlarmActive` AND `txUncertain` untouched, and a probe that ever wrote a non-query byte fails. **Needs on-air validation** (provoke an alarm, confirm it self-clears within a probe interval). Detail retained below. — alarm-driven one-shot `TX;` re-poll (bypassing the `txUncertain` gate, since it is a READ) + an operator "re-check the rig" affordance. The alarm currently blocks the only mechanism that would clear it, so a stuck-TX banner persists until an unsolicited rig push or a pipeline reconnect. **More evidence, never a manual override — no daemon-side ack/clear endpoint** (it would either re-enable keying while the rig may be transmitting or hide the only warning from every tab; dismissal stays UI-local, as `TxAlarmBanner` already does). Mechanism verified in code 2026-07-23; detail in Bugs. _(dogfood 2026-07-21 second stuck-TX incident.)_
...
- _Code-review lows (2026-07-05 `internal/api` review):_ credential-clear asymmetry (forwarder clears on blank, SMTP/lookup keep) — verified open 2026-07-18 (`mergeForwarders` overlays blanks; `mergeSmtp`/`mergeLookupProvider` keep; each only locally documented) · ~~stale `middleware.go` Unwrap comment~~ **FIXED 2026-07-18 (trivials batch)** — now describes the per-write SSE deadline arming → archive at next sweep
- _Code-review nits (2026-07-05 `internal/qsoservice` review):_ ~~`uuid_conflict` unreachable under `force`~~ **FIXED 2026-07-18 (trivials batch)** — `&& !force` dropped; NB force can never hit the dedupe index (random-nonce key), so UNIQUE(uuid) via forced import is the only real collision — pinned by a forced-import test → archive · ~~`importBatchFallback` publish contradiction~~ **FIXED same batch** (doc exception note) → archive · best-effort `contacted_station` cache warm-up uses the request ctx (a detached short-timeout ctx would make it client-independent, like the dedupe refetch) — verified open, still open
...
- _Daemon / data (dogfood triage 2026-07-08):_ downgrade client-abort enrichment WARN→debug when the cause is request-ctx cancellation (verified open 2026-07-18 — `orchestrator.go` `warn` is unconditional, no `ctx.Err()` check) · ~~backport the tightened RST validators (scale + mode-aware) from frontend/app to the shipping logging SPA~~ **MOOT (triage 2026-07-24) — the logging SPA was retired 2026-07-21 (embed + `GET /` route removed; source kept for reference only) and frontend/app already carries the tightened validators, so there is nothing to backport** · ~~rename the Operate "Rig" panel → "Rig Control"~~ **BUILT 2026-07-18** (with the header CAT-chip toggle — `<h3>` + `TILES` registry name → archive at next sweep)
...
- _Dogfood triage 2026-07-23 (frontend/app · all verified in code, detail in Bugs):_ **Rig Control VFO-A/B surface** — swap-vs-select semantics (needs a decision), label not a click target, VFO-B sometimes unrendered (needs on-rig observation) · ~~**Logbook "Not emailed only" is page-local**~~ **BUILT 2026-07-24** (server-side `not_emailed` filter + 3 review rounds) · ~~**Session email resend re-transmits everything**~~ **BUILT 2026-07-24** (session-export not-yet-emailed delta) · ~~**Logbook selection toolbar reflows the table** (reserve the row)~~ **BUILT 2026-07-24** — the toolbar moved into its own `min-h-9` reserved row (`Logbook.svelte`) so selecting a QSO can't wrap the controls row and shove the table down; pure layout, wants a dogfood eyeball after redeploy.
- _Onboarding:_ install / first-run friction for non-Linux operators
- _Diagnostics:_ operator log viewer (DB-manager tab)
- _Code-review lows (2026-07-05 SPA review; re-verified in the 2026-07-18 sweep):_ 13 verified low-severity fixes (the fetch-timeout standout was promoted to P1 and SHIPPED 2026-07-05 → archive) — ~~TX_PWR sub-0.5 W rounding~~ **FIXED 2026-07-18 (trivials batch)** — omit when the value rounds to 0; NB operator: the FTdx10's floor is 5 W, so this was typo-robustness only (real for QRP rigs) → archive · state-reset gaps (**mostly done**: tabCount + freqKnown resets confirmed in `bridge.svelte.ts`; stale-decodes / enrich-zombies unverified — check those two when picked up) · FT8 UI nits (**partially done**: bearing-360° + canAnswer TX-guard confirmed built; drain-abort / FD tooltip / isWorking split unverified)
...
- _Dogfood triage 2026-07-23:_ ~~**Logbook email send has no toasts** (inline `result` only — add toasts, keep the ambiguous-network inline warning)~~ **BUILT 2026-07-24** (`LogbookEmailControls.svelte` — send/outcome toasts + sticky sending→sent supersede; the verbose ambiguous-network warning KEPT inline; regression test `LogbookEmailControls.svelte.test.ts`) · **QTH on the Phone/CW card** — WAI today (QTH lives in the ContactDialog by design); build only if the operator confirms they want it promoted into the Contact-details disclosure.
...
- ~~**P1 · Bridge: the tx-alarm cannot self-clear — alarm-driven re-poll + operator ack.**~~
  **BUILT 2026-07-23 (`internal/bridge/txrecheck.go` + `POST /v1/rig/tx/recheck` → `RecheckTx`
  + the SPA Re-check button; same work as the struck summary entry above). Needs on-air
  validation. Third incident (2026-07-24 triage): the 2026-07-23 TUNE-path stuck-TX ran on a
  build predating this fix → deploy current main. Dummy-load reproduction then refuted both
  the 2 s-duration and FT8-residue hypotheses; RF ingress into the antenna is now the lead
  (companion note below) — fix direction is RF mitigation + the ADR 0057 hardware-PTT avenue,
  not more CAT detection. _Original detail (the design that was built) follows._**
  _(dogfood 2026-07-21 second stuck-TX incident; mechanism verified in code 2026-07-23.)_
  Once `tx_still_keyed` latches, there is no in-process path back to idle on a rig that
  HAS a status query. `confirmTxIdle` is the only clear and it needs an observed
  TXSTATUS; the only issuer of the `TX;` query is `beginTxConfirm`, reached solely from
  an FT8/tune unkey — and `ft8tx.go:105`, `tune.go:152` and `command.go:126` all refuse
  while `txUncertain`, the flag the alarm holds. `read_tx_status` is not in the FTdx10
  rigdef's periodic `READ` set, and `observeRigData`'s liveness fallback is deliberately
  disabled for defs that have a status query. Net: the alarm blocks the only mechanism
...
- ~~**P2 · Logbook "Not emailed only" is page-local, so it looks inert.**~~ **BUILT 2026-07-24**
  (server-side `not_emailed` filter mirroring `missing_from` — daemon `notEmailedMod` + list/count
  handlers + shared `parseBoolQuery`; SPA `toggleNotEmailedOnly` reloads count+page. Plus three
  review rounds: pager refresh on email while filtered, filter-aware empty messages, cross-logbook
  origin guard, non-destructive-on-failure refresh.) _(dogfood
  2026-07-21; confirmed 2026-07-23.)_ `logbook.svelte.ts` filters only the loaded page
  (`this.rows.filter(r => r.sm_fwrd_by_email_status !== 'Y')`) — its own comment says
  "No reload"— while `GET /v1/logbook/{id}/qso` accepts only `limit` / `after` /
  `missing_from`, with no email-status filter. On a multi-thousand-QSO logbook the toggle
  therefore hides a handful of rows and reads as broken. The field is plumbed correctly
  end to end, so this is a scope fix: add a server-side not-emailed filter param
  (mirroring `missing_from`) and drive the toggle off it — or relabel the control to say
  it applies to the current page only.

- ~~**P2 · Session email — "send only not-yet-emailed" delta option.**~~ **BUILT 2026-07-24**
  (`SessionQso.emailed` + `markSessionEmailed`; `ExportDialog` defaults a resend to the
  not-yet-emailed delta via an `onlyUnsent` toggle that resets on each open; the ADIF download
  stays the full session). _(dogfood
  2026-07-21; confirmed 2026-07-23.)_ `ExportDialog.svelte:35` derives `uuids` from the
  WHOLE session (`session.qsos.map(q => q.uuid)`) with no `sm_fwrd_by_email_status`
  filter, and that array feeds both the ADIF export and the email send — so a resend
  re-transmits everything already emailed ("Emailed" markers are informational only).
  Add a delta option (default to not-yet-emailed on a resend). Same status field and same
  operator intent as the logbook filter item above — worth doing in one pass.

- ~~**P3 · Logbook email send has no toasts.**~~ **BUILT 2026-07-24** (send/outcome toasts
  + the sticky sending→sent supersede idiom; the ambiguous-network warning kept inline as
  the primary surface; test `LogbookEmailControls.svelte.test.ts`). _(dogfood 2026-07-21; confirmed
  2026-07-23.)_ `LogbookEmailControls.svelte` reports entirely through local state — the
  button label flips to "Sending…" and the outcome lands in an inline `result` rendered
  in the controls row; no `toasts.*` call, unlike the rest of the app. Add toasts for the
  sending/sent transitions but KEEP the inline text for the ambiguous-network case ("the
  email may still have gone out; check the inbox before re-sending") — that warning must
  not become a 3-second toast.
