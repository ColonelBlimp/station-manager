...
- _Code-review lows (2026-07-05 `internal/api` review):_ credential-clear asymmetry (forwarder clears on blank, SMTP/lookup keep) — verified open 2026-07-18 (`mergeForwarders` overlays blanks; `mergeSmtp`/`mergeLookupProvider` keep; each only locally documented) · stale `middleware.go` Unwrap comment (verified open — the exact stale phrasing is still at `middleware.go:210`)
