...
1. **W-0010 · SELECTED 2026-09-20 — [Improve forwarding, data, and synchronization reliability](work/W-0010-forwarding-data-and-sync-reliability.md).** Token rotation, duplicate-QSO resolution, legal bulk backfill, idempotent outcomes, and bounded reconciliation. **Outcome 9 in progress** (failed-versus-waiting forwarder rows; the preserved QRZ fixture): four slices planned in the dossier; slices 1 (QRZ delete without an upstream id is a no-op, + ADR 0081 success order) and 2 (durable `failure_class`, migration 0011) built 2026-09-20; rulings (a)–(d) settled; slice 3 (boot re-arm of `auth` rows) is next, then 4 (card and API).
