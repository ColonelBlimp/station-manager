...
- _Dogfood triage 2026-07-23:_ **Logbook email send has no toasts** (inline `result` only — add toasts, keep the ambiguous-network inline warning) · **QTH on the Phone/CW card** — WAI today (QTH lives in the ContactDialog by design); build only if the operator confirms they want it promoted into the Contact-details disclosure.
...
- **P3 · Logbook email send has no toasts.** _(dogfood 2026-07-21; confirmed
