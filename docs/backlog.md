...
- **W-0003 · P2 post-ship · OPEN — [Complete the app shell and retire legacy operator SPAs](work/W-0003-retire-legacy-operator-spas.md).** Preserve the remaining config/logbook parity, then remove their routes, embeds, and build gates so `frontend/app` is the sole embedded operator SPA; keep the zero-JS manual separate.
