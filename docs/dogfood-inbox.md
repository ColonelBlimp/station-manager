...
- ~~[2026-06-25] add a radio to config SPA for enabling special ft8 logging~~ **→ backlog** (clarified 2026-06-25 = a toggle in the config SPA FT8 tab for the `ft8.decode_log` JTDX ALL.TXT decode log; tracked: "Config SPA — FT8 decode-log toggle"). Needs daemon GET/PUT surfacing + SPA UI, restart-required.
