...
12. **API surface unchanged from ADR 0005.** Endpoint stays `GET /v1/enrich/callsign?call=X`, single aggregated JSON response, always 200 (per the `enrichment never blocks logging` invariant), AbortController cancellation. The fields returned change to reflect the source split (country fields come from `country`, callsign-class fields come from `contacted_station`).
