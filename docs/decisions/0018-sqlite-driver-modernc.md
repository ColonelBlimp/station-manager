...
- **`mattn/go-sqlite3` is no longer in the dep graph (swap shipped 2026-05-20).** The migrations driver was switched from `golang-migrate/migrate/v4/database/sqlite3` (CGO, blank-imports mattn) to `golang-migrate/migrate/v4/database/sqlite` (pure Go, blank-imports modernc) in `internal/database/sqlite/migrations.go`. Identical `WithInstance(*sql.DB, *Config) (database.Driver, error)` API; the import is aliased `migratesqlite` to avoid the package-name collision with our own `sqlite` package. The new driver wraps each migration in an implicit transaction and rejects explicit `BEGIN`/`COMMIT`; our migrations only use `BEGIN…END` as `CREATE TRIGGER` body delimiters (4 occurrences in `0001_init.up.sql`), not transaction control, so the swap is safe. `go mod tidy` dropped mattn from `go.mod` entirely; `go mod why github.com/mattn/go-sqlite3` reports *"main module does not need package"*; `CGO_ENABLED=0 go build ./...` succeeds. Build is now fully CGO-free for the daemon — the FT8 subsystem (ADR 0021) will reintroduce CGO when its native FFT lands.
...
- **A second SQLite driver gets pulled into the daemon's hot path.** As of 2026-05-20 there is no second driver — the migrations swap to `migrate/v4/database/sqlite` (modernc-backed) closed the previous "two drivers in `go.mod`" state. If a future dependency reintroduces mattn (or any other sqlite driver), the trigger is to either remove it or, if it's load-bearing, switch the project's `IsUniqueConstraintError` helper to match whichever driver is actually registered at runtime. The single-driver invariant is now testable: `go mod why github.com/mattn/go-sqlite3` reporting *"main module does not need package"* is the canary.
