...
The operator has now taken a fresh look at WSJT-X as part of restarting FT8 from scratch. The question that surfaced when planning where the code lives: **what does the new FT8 package actually need from SM?** (Note: this Context paragraph originally framed the restart around a local WSJT-X source checkout as the "Fortran reference." That framing was withdrawn in the same session — see the *Licensing constraint* section below for why WSJT-X source is no longer treated as the implementation reference.)
...
The FT8 work returns to Station Manager as `internal/ft8/`, running in-process inside `cmd/smd`. The package follows the same daemon-subsystem pattern as `internal/bridge` (ADR 0013, ADR 0019) — same `Initialize` / `Start(ctx)` / `Stop()` lifecycle, same package-boundary discipline, same enabled-flag gating. The implementation is built **from the FT8 protocol specification** (Joe Taylor's QEX 2020 paper and companion academic references — see the *Licensing constraint* section below for why this matters and what's safe vs not), then validated through a **layered test architecture** (see `docs/v2-design/milestones.md` § Milestone 4 design preamble): spec-vector tests for each algorithmic primitive (Layer 1), encoder ↔ decoder round-trip tests (Layer 2), `ft8sim`-generated synthetic signals with known truth (Layer 3), operator-recorded real signals against captured `.expected` fixtures (Layer 4), and a one-shot `cmd/ft8-corpus-prep` developer tool that uses `jt9` to generate Layer 4 fixtures (Layer 5). The critical property: **`jt9` is never a test-time dependency** — it enters only at Layer 5 during corpus preparation, and the operator-facing daemon has zero dependency on WSJT-X being installed. Goal: same or better decode results than WSJT-X under the same conditions.

**Note on revision:** an earlier draft of this section described the WSJT-X source as "the Fortran specification" and committed to "exact port first." That framing was withdrawn 2026-05-17 once the GPL v3 vs MIT licence gap was surfaced — line-by-line translation of GPL source produces derivative work even across languages, which would force-relicense SM. The current wording reflects the corrected position; full reasoning in the *Licensing constraint* section.
...
- **Daemon binary size grows.** The FT8 implementation plus LDPC tables, codebooks, and CGO bridges will add MB to the binary. Acceptable — the RPM is already operator-installed, not network-distributed at scale.
- **CGO surface introduced (committed).** A pure-Go FT8 decoder is possible but native FFT and LDPC implementations measurably outperform their Go alternatives inside the 15s slot budget — see the *CGO commitment* paragraph in `docs/v2-design/milestones.md` § Milestone 4 for the rationale. WSJT-X happens to use `libfftw3` + `portaudio`, but FFTW3 is GPL v2 and is excluded here per the *Licensing constraint* section; SM uses KissFFT or PocketFFT (both BSD-3) for FFT, PortAudio (MIT-equivalent) for audio. CGO complicates cross-compilation and increases build-time dependencies. Caught early: the CD pipeline fails loudly on CGO build issues.
...
- **Spec-implementation decode quality lags WSJT-X materially.** If our from-spec implementation produces measurably worse decode results than `jt9 -8` against the M4.1 Layer 3/4 corpus (decoder threshold worse than the QEX paper's −19.6 dB AWGN benchmark by more than ~1 dB, or systematic message-misdecodes on clean signals), the trigger is "characterize the deviation, then either fix-to-parity or accept the difference and document it." Goal stays "same or better than WSJT-X." The diagnosis path goes through the layered test architecture (`docs/v2-design/milestones.md` § Milestone 4 design preamble): if Layer 1+2 are green, the gap is in signal-processing (sync / demod / FFT) rather than the algorithmic primitives.
- **Go runtime ceiling on the BP/OSD/AP hot path.** Surfaced 2026-05-29 (Session 104) after the sandbox AP3 implementation produced a ~11× per-slot slowdown (3.5 s → 38 s) at zero matched-recovery on the working corpus. The structural gap is portable SIMD for batched-message LDPC BP — Go's stdlib doesn't deliver it without inline assembly stubs, and CGO transition overhead rules out fine-grained C kernel calls (per `feedback_cgo_scalar_interop_overhead`). If decode-depth work continues to hit this wall (OSD-3, multi-pass hash-feeding restart, further AP forms), the answer is "C/Zig hot-path core called from Go orchestration" — not "more pure-Go optimisation." Trigger fires when (a) operator decides decode parity matters more than language uniformity, or (b) a real performance pain point appears in operating use. Full conversation captured in `docs/session-handoff.md` Session 104 item 11 and memory `project_sm_ft8_language_question`. The sandbox stays Go until a deliberate decision lands.
...
- **Steve Franke (K9AN), Bill Somerville (G4WJS), Joe Taylor (K1JT): "The FT4 and FT8 Communication Protocols," QEX July/August 2020.** The canonical protocol specification — implementation source-of-truth per the *Licensing constraint* section. Hosted by the WSJT-X project on SourceForge: <https://wsjt.sourceforge.io/FT4_FT8_QEX.pdf> (free, no paywall). Awarded the ARRL Doug DeMaw W1FB Technical Excellence Award.
...
- Memory `project_ft8_library.md` — reflects the in-SM reversal and the spec-implementation framing (NOT Fortran translation) per the *Licensing constraint* section.
- Memory `project_sm_cd_pipeline_planned.md` — CD pipeline shipped session 66; gates FT8 commits from commit #1.
- WSJT-X distribution (Fedora `wsjtx` / Debian `wsjtx` / etc.) — source of the `jt9` and `ft8sim` binaries used as black-box parity oracles. The source tree is **not** consulted as an implementation reference (see *Licensing constraint*); only the packaged binaries are exec'd.
