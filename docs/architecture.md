...
| Station Events | [`internal/stationevents`](../internal/stationevents), [`internal/stationevents/recorder`](../internal/stationevents/recorder) | `stationevents` is the standard-library-only vocabulary (closed category/kind pairs, severities, typed facts). Bridge and FT8 report facts through injected observers to the assembly-owned recorder, whose bounded queue keeps database writes off their paths. The forwarding worker and browser export-failure POST write notification rows directly through the store. |
...
| Durable operator events (Station Events) | The `operator_event` table in the log SQLite database, written only by the store's own best-effort transaction | Newest 500 rows per category, oldest-first eviction. Writes never join a QSO transaction. Bridge and FT8 enqueue without waiting on SQLite; if the recorder queue fills, it drops each newest fact with a warning. Direct notification writes may wait on the store. `smd.log` stays the independent diagnostic record. |
